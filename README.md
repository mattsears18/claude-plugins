...
2. **Refines work** — `/refine-issues` is a source-branched refiner: raw user-feedback issues get classified (already-done / decline / legitimate) and rewritten into implementation-ready tickets; Claude-filed feature requests with `## Open questions` get reasonable defaults committed; everything else with no recognizable refinement pattern falls through to `needs-human-review`. The user-feedback path is gated by a `needs-human-review` label so no code-modifying agent runs until a human signs off; the open-questions path is dispatch-eligible immediately.
...
### Dispatch substrate — Agent-tool dispatch (default) + Dynamic Workflows (alternate)

Every `mode:`-driven worker is dispatched via Claude Code's `Agent` tool with `isolation: "worktree"` — the harness provisions and cwd-pins the worktree in response, and a `PreToolUse` hook hard-fails any guarded dispatch that omits the isolation parameter. This is the **default, primary dispatch shape** as of [#825](https://github.com/mattsears18/shipyard/issues/825), which restored it after the prior [Dynamic Workflows](https://code.claude.com/docs/en/workflows) migration ([#782](https://github.com/mattsears18/shipyard/issues/782)) found the `Workflow` runtime's dispatched workers could not perform a single file write. A deterministic JavaScript orchestration script (`plugins/shipyard/workflows/do-work-dispatch.workflow.js`) remains available as a **documented alternate substrate** — reversible on evidence, not removed — for the same seven `mode:` values; because that runtime's `agent()` primitive has no isolation option of its own, the orchestrator pre-provisions each worker's git worktree itself (`git worktree add`) and passes the path as that work unit's `worktreePath` when this alternate is used. Either shape's dispatch returns a **schema-validated structured result** (`plugins/shipyard/schemas/worker-return.schema.json`) that the orchestrator translates back into the free-text return vocabulary before reconcile, so a malformed or ambiguous return fails loudly at the stage boundary instead of being re-parsed from prose. See [`dispatch-rules.md`'s "Dispatch shapes" section](plugins/shipyard/commands/do-work/dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) — the authoritative spec — for the full default/alternate split and per-mode call-site details, rather than re-deriving the mechanics here.

The `Agent` tool is *also* used for dispatches that never carry a `mode:` value — the opt-in verify gate, the epic-decomposition worker, and the read-only scope-preflight / refinement workers — but it is no longer exclusive to them; it's the default shape for the seven `mode:`-driven workers too.

> **⚠️ Upgrading from ≤ 3.1.x:** the `dispatch.substrate` config knob is **removed as of `4.0.0`** and was **not** reintroduced when #825 restored `Agent`-tool dispatch as the default — there's no flag to set either way, but there are still two documented dispatch shapes (see above), not one. Delete any `dispatch` block from `shipyard.config.json` / `.shipyard/config.local.json` before upgrading: the config schema is `additionalProperties: false`, so a layer that still carries one now **fails validation** (`/shipyard:config validate` reports it; `shipyard-config.sh load` exits 70) instead of being silently ignored.

### Label conventions

Shipyard treats several label families as load-bearing — origin labels (`user-feedback`, `audit:<dimension>`), the session-stamp label (`shipyard`), state labels in the `blocked:*` namespace (`blocked:agent-soft`, `blocked:ci`), and gate labels (`needs-human-review`, `needs-operator`, `needs-triage`). `needs-operator` (browser/console actions `/do-work` drives itself by default via `claude-in-chrome` — see the [Quick start](#quick-start) `/do-work` example above) is distinct from `needs-human-review` (genuine human decisions). The canonical reference lives in [`CLAUDE.md`'s "Label conventions" section](./CLAUDE.md#label-conventions) — that's the source of truth; this README intentionally doesn't duplicate it.
...
plugins/shipyard/
  ...
  workflows/                   # Dynamic Workflows dispatch substrate (do-work-dispatch.workflow.js) — see
                               #   "Dispatch substrate" above; a documented alternate to the default Agent-tool shape
