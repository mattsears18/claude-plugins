...
- **Gate labels** ([CLAUDE.md → Gate labels](./CLAUDE.md#gate-labels-intake--human-review)) — `needs-human-review`, `needs-operator`, `needs-triage`. Gate issues through human review before any code-modifying agent runs. (Refinement is now a just-in-time source-signal scan rather than a persisted `needs-refinement` gate label — eliminated in [#520](https://github.com/mattsears18/shipyard/issues/520).)
