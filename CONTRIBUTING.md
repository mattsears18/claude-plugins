...
- **Use [`plugins/shipyard/.claude-plugin/plugin.json`](./plugins/shipyard/.claude-plugin/plugin.json) as the structural reference.** It carries the manifest fields (`name`, `version`, `description`, `author`, `homepage`, `repository`, `license`, `keywords`, `permissions`). Note that the `permissions.deny` block listing the hook-bypass flags (`--no-verify`, `--no-gpg-sign`, `--no-commit-hooks`) is **not** a verified enforcement mechanism — `permissions` is not among the documented plugin-manifest fields, and the documented permission rule sources name no plugin-manifest tier ([#1511](https://github.com/mattsears18/shipyard/issues/1511)). The flags are actually refused by [`plugins/shipyard/hooks/refuse-hook-bypass-flag.sh`](./plugins/shipyard/hooks/refuse-hook-bypass-flag.sh); don't model a new plugin's safety guarantees on the manifest block.
