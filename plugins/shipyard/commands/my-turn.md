...
- **The human-only queue is empty** — every ranked human-only item has been walked (handled or skipped). Print the [empty state](#empty-state) confirmation, plus the [operator pointer line](#operator-pointer-line) if any `agent-console` items were filtered out, and stop.
