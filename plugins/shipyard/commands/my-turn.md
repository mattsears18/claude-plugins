...
- **P1 — decisions**
  ...
  - Issues with `agent-console` (or the legacy `needs-operator` name), **only when `my_turn.assume_operator_enabled: false`** ([#1093](https://github.com/mattsears18/shipyard/issues/1093)) — same "blocked on a human because nothing is currently draining it" shape as `blocked:ci` above; excluded entirely under the default config (see the Pass B bucket above)
