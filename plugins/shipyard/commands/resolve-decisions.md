...
This command owns the **decision walkthrough** that [`/shipyard:my-turn`](./my-turn.md) reuses. `/my-turn` collects the decisions across **every** decision-gated issue in one uninterrupted run, driving *this* command's interactive per-decision flow inline (see [`/my-turn`'s decision-gated walkthrough](./my-turn.md#decision-gated-walkthrough)) rather than reinventing it ([#566](https://github.com/mattsears18/shipyard/issues/566), [#635](https://github.com/mattsears18/shipyard/issues/635)) — with the record-and-unblock mutation [deferred](#firing-mode--immediate-default-vs-deferred-batch-caller) to its batched commit phase so no GitHub round-trip lands between two of the maintainer's answers ([#1070](https://github.com/mattsears18/shipyard/issues/1070)). Run `/resolve-decisions` directly to resolve a specific decision-gated issue without going through `/my-turn`; that path records immediately, as it always has. Either way, the per-decision walkthrough and the record-and-unblock mutation are defined here — the single source of truth for both entry points.
...
### Firing mode — immediate (default) vs deferred (batch caller)

This step has **two firing modes**. The recorded content is identical in both; only *when* the mutation fires differs.

- **Immediate (default).** A direct `/resolve-decisions` invocation records as soon as the walkthrough for its one issue finishes — post the comment, remove the gate, confirm. This is the standalone behavior and it is unchanged.
- **Deferred (batch caller).** When a batching caller drives this walkthrough — [`/my-turn`'s collect phase](./my-turn.md#phase-1--collect-no-mutation-no-work) is the only one today — the walkthrough **emits the decision set as a value and performs no mutation at all**. The caller journals it and fires every issue's record step together in its own commit phase. This is what keeps the maintainer's answer-to-next-question latency free of GitHub round-trips ([#1070](https://github.com/mattsears18/shipyard/issues/1070)).

**Deferring changes nothing about the decision content, the comment body, the partial-run rule, or which labels come off.** A batch caller MUST apply the same [answered](#when-every-blocking-decision-was-answered) / [partial](#when-some-decisions-were-skipped-or-the-walkthrough-was-halted) branch, per issue, that an immediate run would have applied — batching is a change in *timing*, never in *outcome*. A caller that defers is responsible for eventually firing the record step for every issue it collected; abandoning a journaled decision set without recording it silently loses the maintainer's answers.

The rest of this section describes **what** gets recorded — the contract both firing modes satisfy.
...
- **Don't advance past an unanswered decision.** Walk one at a time; don't dump all N questions at once. Each locked answer informs the next (part 4) — that carry-forward is lost if you ask everything simultaneously. **A batching caller does not get an exemption from this**: `/my-turn`'s collect phase may group *mutually independent* decisions into one `AskUserQuestion` call, but any decision whose options depend on an earlier answer is still asked singly, in source order. Batching removes the *work* between questions, never the sequencing that carry-forward needs ([#1070](https://github.com/mattsears18/shipyard/issues/1070)).
