...
> **Interim fall-through home ([#520](https://github.com/mattsears18/shipyard/issues/520)).** The fall-through previously swapped `needs-refinement` → `needs-triage`; with both labels being collapsed, an issue with **no recognizable refinement pattern** now lands on **`needs-human-review`** — but ONLY this genuine no-automated-path subset, never the auto-processable classify+rewrite / resolve-defaults work (folding *those* into the inert human-gate would strand refinable work).
>
> **Staying on `needs-human-review`, not investigate-mode ([#857](https://github.com/mattsears18/shipyard/issues/857)).** [#514](https://github.com/mattsears18/shipyard/issues/514) (investigate-mode runtime) and [#556](https://github.com/mattsears18/shipyard/issues/556) (orchestrator dispatch wiring) have both shipped — a trusted-author untriaged-looking issue already routes to `investigate_candidates` on its own detection signals (see [`do-work/setup/04-backlog-divert.md`](do-work/setup/04-backlog-divert.md); the `needs-triage` label was retired in [#1120](https://github.com/mattsears18/shipyard/issues/1120)). But Branch C below is a heterogeneous bucket, not a pure bot/untriaged one: it also catches `resolve-defaults` escalations that are unresolvable *because* they need human business/policy judgment (P0-vs-P1, deprecate-or-not, marketing copy) — never investigate-mode material, since investigate-mode's job is disposition-ing a bug/crash report, not answering a policy question. Routing the whole branch to investigate-mode would misroute that second class, so the fall-through stays on `needs-human-review` until someone splits Branch C to let only the genuinely bot/untriaged subset fall through to investigate-mode's detection scan. That split is a real design change, not a stale-comment cleanup — file a fresh issue for it if it's worth doing.
...
>
> → Post a comment whose first line is the shared `<!-- do-work-refinement-agent -->` dedupe sentinel (per the instruction above) and whose **second** line is the additional `<!-- do-work-refinement-fallthrough -->` provenance TRIGGER marker — distinct from the dedupe sentinel, and load-bearing: it's what lets `/my-turn` distinguish this branch's `needs-human-review` from the other seven provenances that land on the same label (issue [#1091](https://github.com/mattsears18/shipyard/issues/1091)). The classify+rewrite and resolve-defaults branches do NOT add this second marker — only fall-through does. Body: `"<!-- do-work-refinement-agent -->\n<!-- do-work-refinement-fallthrough -->\nNo automated refinement rule applies to this issue. Surfacing to a human via \`needs-human-review\` (see /shipyard:my-turn). Add a refinement rule in commands/refine-issues.md if this becomes a recurring shape."`
...
>
> **Why `needs-human-review` and not `needs-triage` ([#520](https://github.com/mattsears18/shipyard/issues/520)).** This is the interim fall-through home during the binary-backlog collapse — only the genuine no-automated-path subset lands here, never the auto-processable classify+rewrite / resolve-defaults work. See the **"Staying on `needs-human-review`, not investigate-mode"** callout under [Source-branched refiner](#source-branched-refiner) above for why this branch doesn't route to investigate-mode even though that routing has since shipped ([#857](https://github.com/mattsears18/shipyard/issues/857)) — Branch C mixes bot/untriaged issues with unresolvable human-judgment escalations, and only the former belongs there.
>
