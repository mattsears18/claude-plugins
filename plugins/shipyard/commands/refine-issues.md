...
>
> **Staying on `needs-human-review`, not investigate-mode ([#857](https://github.com/mattsears18/shipyard/issues/857)).** [#514](https://github.com/mattsears18/shipyard/issues/514) (investigate-mode runtime) and [#556](https://github.com/mattsears18/shipyard/issues/556) (orchestrator dispatch wiring) have both shipped — a trusted-author `needs-triage` issue already routes to `investigate_candidates` (see [`do-work/setup/04-backlog-divert.md`](do-work/setup/04-backlog-divert.md)). But Branch C below is a heterogeneous bucket, not a pure bot/untriaged one: it also catches `resolve-defaults` escalations that are unresolvable *because* they need human business/policy judgment (P0-vs-P1, deprecate-or-not, marketing copy) — never investigate-mode material, since investigate-mode's job is disposition-ing a bug/crash report, not answering a policy question. Routing the whole branch to investigate-mode would misroute that second class, so the fall-through stays on `needs-human-review` until someone splits Branch C to route only the genuinely bot/untriaged subset to `needs-triage`. That split is a real design change, not a stale-comment cleanup — file a fresh issue for it if it's worth doing.
