...
> **Binary-backlog re-key ([#519](https://github.com/mattsears18/shipyard/issues/519)).** Epic-decomposition handoffs ride `needs-human-review` **+ the `<!-- do-work-needs-decomposition -->` body marker** (stamped on the scope-agent diagnosis comment by [`do-work/setup.md` step 6](./do-work/setup/06-scope-preflight.md#6-initial-scope-pre-flight)) — the former dedicated `needs-decomposition` / `tracking` label pair was folded into `needs-human-review` (see CLAUDE.md → Label conventions → Epic-decomposition handoff). The candidate fetch is `--label needs-human-review` filtered to issues carrying that trigger marker, so `/decompose-epic` finds exactly the epics it used to, never the unrelated human-gated issues that now share the same label.

`/decompose-epic` is the **explicit, human-invoked** entry point — it mirrors [`/refine-issues`](./refine-issues.md)' shape (sentinel-keyed idempotency, parallel per-issue workers, untrusted-input discipline) and is how you decompose in **bulk** or on **demand** (all eligible epics at once, or a named `--issue N`). As of [#665](https://github.com/mattsears18/shipyard/issues/665) its [Worker prompt template](#worker-prompt-template) is **also reused inline by `/do-work`**: when a scope-preflight defer is `confirmed-non-shippable-as-single-PR` with a mechanically-decomposable `evidence_pointer` (`Multi-PR sequence:` / `Missing dependency:`), `/do-work` inline-invokes that same worker logic to shard the epic right then — no manual round-trip. Inline auto-decompose is the **default** for the mechanical classes and is gated by the `decompose.auto` config knob (default `true`; set `false` to opt out and restore the park-for-manual-`/decompose-epic` behavior). This command remains the single source of truth for the sharding — see [The auto path (inside `/do-work`)](#the-auto-path-inside-do-work).
...
**Built and default-on for the mechanical classes as of [#665](https://github.com/mattsears18/shipyard/issues/665).** `/do-work`'s scope-preflight/defer path ([`do-work/setup/06c-scope-handling-ui.md` Recording-path step 5](./do-work/setup/06c-scope-handling-ui.md#handling-each-returned-entry-fires-as-each-background-agent-completes), plus the [drain 5.a/5.b re-validation](./do-work/drain.md#5a--re-validate-orchestrator-judgment-entries)) inline-invokes **this command's [Worker prompt template](#worker-prompt-template) verbatim** when a scope agent returns `confirmed-non-shippable-as-single-PR` with a mechanically-decomposable `evidence_pointer` (`Multi-PR sequence:` / `Missing dependency:`). The worker is the **single source of truth** for the sharding — the inline path re-derives nothing; it just dispatches the same `shipyard:decompose-worker` decomposition worker (no worktree) with `--max-subissues <decompose.max_subissues>`. This is the same reuse posture as `/refine-issues` being the single source of truth for the refinement logic that `/do-work` step 3.5 invokes.

**What gates it (config).**

- `decompose.auto` (boolean, default `true`) — when true, mechanical-class confirmed-non-shippable epics are auto-decomposed inline; when false, they fall back to the pre-#665 behavior (park with `needs-human-review` + the `<!-- do-work-needs-decomposition -->` trigger marker and wait for an explicit `/decompose-epic` run). This is the **opt-out** for anyone who wants a human to choose to run the decomposition.
- `decompose.max_subissues` (integer, default `8`) — the hard cap the worker's confidence gate (Step B) enforces on both the inline path and the standalone `--max-subissues` flag.

**Why auto-firing is safe now (the guardrails that replaced the explicit-command gate).** The original deferral concern was runaway issue creation from a misclassified epic sharded into a fan-out of vague sub-issues every session. Three mechanisms bound that risk without a human in the loop: (1) **only two mechanically-decomposable evidence classes** are ever auto-attempted (`Multi-PR sequence:` / `Missing dependency:`) — the non-mechanical classes still defer to the human handoff; (2) the worker's **confidence gate** (Step B) escalates rather than shards whenever a phase isn't single-PR-sized, the ordering is ambiguous, or a sub-issue's acceptance criteria aren't codebase-supported; (3) the **`--max-subissues` cap** escalates rather than fanning out. A misclassified or low-confidence epic hits the escalate path (Step D) — it lands exactly where it would have without the auto path (a `needs-human-review` handoff surfaced by `/my-turn`), so the auto path is strictly no-worse than the manual park for the hard cases and strictly better for the confident mechanical ones. The **idempotency sentinel** (`<!-- do-work-decompose-agent -->`) prevents the inline path and a later standalone run from double-sharding the same epic.

## Don't

- Don't re-implement or fork the sharding logic. This command's [Worker prompt template](#worker-prompt-template) is the **single source of truth** — the inline auto-decompose path ([#665](https://github.com/mattsears18/shipyard/issues/665)) reuses it verbatim rather than duplicating it. If the sharding behavior needs to change, change it here and both call sites (the explicit command and `/do-work`'s inline invocation) pick it up. (Historical note: pre-#665 this command was explicit-command-only and auto-firing inside `/do-work` was forbidden. #665 built the auto path — default-on for the mechanically-decomposable classes, gated by the `decompose.auto` opt-out — so the prohibition is gone; the guardrails that made it safe are documented in [The auto path (inside `/do-work`)](#the-auto-path-inside-do-work).)
