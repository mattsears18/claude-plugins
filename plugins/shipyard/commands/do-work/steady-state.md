...
- **reaped: my worktree was reaped while I was running** — the worker's worktree was torn down mid-run by the cleanup logic. This is external-infrastructure noise, NOT a logic failure. **Do NOT add `blocked:agent`.** Instead:
  ...
  3. **When `backlog.self_assign` resolves `true`** (config default `false`, issue [#1248](https://github.com/mattsears18/shipyard/issues/1248)), remove the `@me` assignee so the fresh dispatch can re-claim cleanly: `gh issue edit <N> --repo <owner/repo> --remove-assignee @me 2>/dev/null || true`. Skip this call when `backlog.self_assign` is `false` (the default) — no assignment was ever written, so there is nothing to remove.
...
**Purpose narrows under `backlog.respect_assignees: false` (issue [#1248](https://github.com/mattsears18/shipyard/issues/1248), the default).** The divergence-smell bullet above assumes `classify`'s assignee-based drop clause is live — true only when `backlog.respect_assignees == true`. With the default (`false`), `backlog-filter.sh classify` never drops on assignee at all (see [scripts/backlog-filter.sh](../../scripts/backlog-filter.sh)'s `--respect-assignees` flag), so `me_assigned_open` can no longer be read as "the bucket a wrong assignee filter erases" — there is no assignee filter to get wrong. The token is NOT removed: it remains a general-purpose count of self-assigned open issues (useful on its own, and consumed by [`04f-completion-ledger.md`](./setup/04f-completion-ledger.md)'s divergence check independent of any filter-defect purpose), and the original divergence-smell reading is fully restored the moment a repo opts into `backlog.respect_assignees: true`.
