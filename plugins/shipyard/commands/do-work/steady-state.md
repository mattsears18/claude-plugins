...
Otherwise, the refresh is **event-driven with adaptive backoff** (see [refresh trigger rules](#refresh-trigger-rules) below). When a refresh fires, it runs four sub-steps (plus, on a merge-completion trigger, the [own-the-tail sweeps](#d-tail-own-the-tail-merge-completion-sweeps-phase-c--663) in D-tail):
...
- **`merge == "BLOCKED"` AND `pending == 0` → this is `red`, not queue latency ([#1435](https://github.com/mattsears18/shipyard/issues/1435)).** Zero pending checks while still `BLOCKED` means CI has finished having its say — a healthy PR's `BLOCKED` clears the instant its last pending check resolves, so a `BLOCKED` PR with nothing left pending is definitionally stuck on either an explicit failure or a required-check context that never ran (the same second discriminator [step 5's failed-PR scan](./setup/04-backlog-divert.md#5-snapshot-failing-prs) applies). Push `<M>` onto `failed_prs` (deduped against entries already there or already in `in_flight`) so the normal step-C dispatch picks it up as `fix-checks-only` on this same turn if a slot is free, rather than waiting for the next 5-minute refresh. Log `[blocked-classify] PR #<M> is BLOCKED with 0 pending checks (<fails> explicit failure(s)) — not queue latency, enqueuing fix-checks-only (#1435)`. **`merge == "BLOCKED"` AND `pending > 0`** → this is genuinely `pending` (healthy queue wait) — no-op this sweep, same as before.
