...
| [Step C dispatch](./steady-state.md#c-dispatch-a-replacement-if-work-remains--mandatory-action) | `in_flight` (new slot), `ready_issues` (consumed), `failed_prs` (consumed), `soft_caps` (increment), `raw_backlog` (post-refill), `unfiltered_open_count` / `me_assigned_open` / `last_fresh_fetch` (#1246, from the same lightweight backlog re-check's wide fetch) | every dispatch |
