...
```json
{
  ...
  "ci_capacity": { "shape": "self-hosted", "pool_total": 4, "queued_at_start": 6 },
  "main_ci": {
    "status": "green",
...
|---|---|---|
| [Step 0.5 → step 1.5](./setup/01-repo-recovery.md#15-initialise-the-session-state-file) | Session file created with `init`; `ci_capacity` written through immediately after (from [step 1.36](./setup/01-repo-recovery.md#136-detect-ci-executor-pool-capacity-and-clamp-toward-it-1141)'s read) | once, at startup |
