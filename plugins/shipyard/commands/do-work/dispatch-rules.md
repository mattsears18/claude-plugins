...
**Per-mode routing.** Model tiering is **role-based** ([#784](https://github.com/mattsears18/shipyard/issues/784)): implementation defaults to the cheap **Sonnet 5** agent-runner, the mechanical fix modes pin **Haiku** (cheaper still — mostly pattern-matching against failing logs), and the strong, harder-to-fool **Opus 4.8** tier is reserved for the verify gate, where the highest-stakes judgment earns its price. See [RATIONALE → Model-tiering cost rationale](../do-work-RATIONALE.md#dispatch-rules--model-tiering-cost-rationale-157-784) for the cost/quality data behind the tiering.
