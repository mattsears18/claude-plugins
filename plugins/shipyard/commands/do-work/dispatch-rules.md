...
Both shapes run the **identical** per-mode prompt template documented later in this file and produce the **identical** per-mode return-string vocabulary — only the invocation mechanics differ, and the decision tree below (steps 1–6: which candidate to run, and every mode-specific augmentation) is shape-agnostic. **Why two shapes, and why `Agent`-tool is the default:** the `Agent`-tool + `isolation: "worktree"` path was the sole dispatch mechanism before the [#782](https://github.com/mattsears18/shipyard/issues/782) Dynamic Workflows migration, was retired as a dispatch target in [#791](https://github.com/mattsears18/shipyard/issues/791), and is restored here as the default in [#825](https://github.com/mattsears18/shipyard/issues/825) after a controlled retest showed a `Workflow`-substrate-dispatched worker's `Edit`/`Write` calls refused outright — a harness-level "parent bg session hasn't isolated" state that the `worktreePath` pre-provisioning contract cannot convey to the write-guard (see #825 for the full repro and the two-orchestrator-isolation-states retest that ruled out orchestrator-side isolation as the cause). The `Workflow` shape stays documented as an alternate rather than being deleted: the choice is reversible on evidence — if a per-`agent()` isolation primitive lands upstream, it can be re-promoted, this time behind a live end-to-end dispatch that actually writes a file (the acceptance gate #791 skipped).

**Per-mode routing.** Model tiering is **role-based** ([#784](https://github.com/mattsears18/shipyard/issues/784)): implementation defaults to the cheap **Sonnet 5** agent-runner, the mechanical fix modes pin **Haiku** (cheaper still — mostly pattern-matching against failing logs), and the strong, harder-to-fool **Opus 4.8** tier is reserved for the verify gate, where the highest-stakes judgment earns its price. Vantage's cost analysis makes tiering a top cost lever (~50–70% claimed savings vs all-Opus) *and* a quality win (Opus 4.8 is ~4× less likely than 4.7 to silently pass flawed code). See [#157](https://github.com/mattsears18/shipyard/issues/157) for the original cost rationale and [#784](https://github.com/mattsears18/shipyard/issues/784) for the tier-by-role refresh.
...
| `fix-rebase`             | `shipyard:fix-rebase-worker`     | [`issue-worker/fix-rebase.md`](../../agents/issue-worker/fix-rebase.md)                       | `haiku`             | Git mechanics — fetch + rebase + force-with-lease.                |
...
**Per-dispatch model resolution — honor `models.<mode>` ([#727](https://github.com/mattsears18/shipyard/issues/727)).** The plugin's built-in per-mode defaults (the frontmatter pins on the shim agents) are plugin-owned, so a consumer repo cannot edit them in place — which is precisely what the `models.*` config block exists for. Until #727 that block was a **dead surface**: schema-defined, settable via `/shipyard:config set models.issue_work <id>`, documented in CLAUDE.md, covered by tests — and read by nothing, so a user who set `models.issue_work: claude-sonnet-4-6` to cut spend silently kept dispatching on the frontmatter/session default, with no warning anywhere. **Every** dispatch in the table above — step 7's initial pool fill and step C's replacement dispatch alike — now resolves the mode's model from the merged config:
...
2. **`failed_prs` non-empty?** → pop the front entry. Path-collision rules don't apply (you're working an existing PR's branch, not a new path claim).
   ...
   **For `fix-rebase` dispatches (drain-phase only — see [end-of-session drain](./drain.md#end-of-session-drain)):** the same `failed_prs`-style branch-targeted dispatch shape, but a different prompt template and a different return contract. Use `mode: fix-rebase` (Haiku-pinned).
...
3. **`ready_issues` non-empty?** → scan from the head for the first entry whose `claimed_paths` **don't collide** with any entry in `in_flight`, per the two-tier collision rule below.
   ...
   **If spike-shaped** → dispatch `mode: spike` instead of `mode: issue-work` (default model — same tier as issue-work, no cheaper pin; see [`spike-worker.md`'s "Why no model pin"](../../agents/spike-worker.md#why-no-model-pin) for the reasoning, which the `models.spike` default carries forward). Read the fan-out cap from the merged config:
   ...
   **This template is the prose spec of the prompt, dispatch-shape-agnostic.** Under the default `Agent`-tool shape it's passed verbatim as the `prompt` parameter (see [Agent-tool dispatch](#agent-tool-dispatch--the-default-dispatch-shape-825)); under the `Workflow`-substrate alternate it's what `buildIssueWorkPrompt` in [`do-work-dispatch.workflow.js`](../../workflows/do-work-dispatch.workflow.js) assembles from the same augmentation fields, passed instead as `args.issues[0]` fields. The augmentations below (verify-gate, user-feedback preamble, phase-1 slice, next-available-version) are computed **by the orchestrator** either way. Resolve every augmentation first, then hand the whole set to whichever shape's call per the [dispatch shapes](#dispatch-rules-used-by-step-7-and-step-c) above. Keep this template and every mode's builder in sync — this file is the reviewable prose, the builders are what the alternate `Workflow`-substrate shape runs, and full de-duplication isn't possible (that runtime has no filesystem access to `require`/`import` a shared source — verified against the identical constraint [#856](https://github.com/mattsears18/shipyard/issues/856) hit for the worker-return schema). A drift on mode inventory, `agents/issue-worker/<mode>.md` spec-file references, or the verify-gate/user-feedback/phase-1-slice/next-available-version augmentation anchors is caught mechanically by [`scripts/check-dispatch-prompt-parity.mjs`](../../scripts/check-dispatch-prompt-parity.mjs) (run via [`scripts/tests/dispatch-prompt-parity-880.test.sh`](../../scripts/tests/dispatch-prompt-parity-880.test.sh) in CI) — issue [#880](https://github.com/mattsears18/shipyard/issues/880): the two copies had already drifted (the user-feedback preamble's trailing "the refinement step may have misread the user" clause was missing from the builder), with nothing catching it.
...
2. **`isolation: "worktree"`** — mandatory on every dispatch. The harness provisions and cwd-pins the worker's worktree in response; [`enforce-worktree-isolation.sh`](../../hooks/enforce-worktree-isolation.sh) hard-fails a dispatch of any guarded `subagent_type` that omits it (#293 / #514 / #774).
...
**Restored as the default in [#825](https://github.com/mattsears18/shipyard/issues/825), after having been retired as a dispatch target in [#791](https://github.com/mattsears18/shipyard/issues/791).** The `#782` Dynamic Workflows migration ran in five phases: #787 committed the `Workflow`-substrate script as inert scaffolding; #788 wired `issue-work` to it; #789 wired the remaining six modes; #790 flipped the built-in `dispatch.substrate` default from `"agent"` to `"workflow"` (the 3.0.0 major bump) while retaining this `Agent`-tool path for one release as an instant-revert override; #791 retired that override and deleted the `dispatch.substrate` knob entirely, making the `Workflow` substrate the sole path. #825 found that sole path could reach a `mode:`-driven worker through every step — self-assign, read, verify, `git rev-parse` — right up to the first `Edit`/`Write` call, which the harness refused with a "parent bg session hasn't isolated yet" error; a controlled retest (dispatching the identical work unit with the parent orchestrator isolated vs. not) reproduced the refusal both ways, ruling out orchestrator-side isolation as the fix. Because `Workflow`-substrate dispatch could not perform a single file write in either configuration, #825 restores this `Agent`-tool shape as the default rather than leave shipyard's autonomous code-writing loop non-functional pending an upstream harness change with no ETA. **There is no config knob choosing between the two shapes** — the choice is spec-level (this section is the default; the next section is the alternate), not something `dispatch.substrate` or any other config key selects; re-adding that knob would itself be a breaking schema change (the repo/local config schema is `additionalProperties: false`), and the decision doesn't need to be per-repo configurable.
