...
**Agent-tool shape only for now.** The sentences above are additive text on the `Agent`-tool shape's prose Context paragraph — the primary, load-bearing dispatch path (per the "two dispatch shapes" note at the top of this file) — and require no companion edit for `check-dispatch-prompt-parity.mjs` to stay green (this paragraph is not a `**<Name> augmentation**`-headed block the checker derives its anchor set from). The `Workflow`-substrate alternate's `worktreeAnchorLines` helper does **not** yet render either sentence — wiring it through (threading `pluginRootStale`/`skillCacheStale` onto the work unit, alongside `pluginRoot`) is tracked as a follow-up rather than done here, together with the deeper structural fix (making every worker's spec-file read resolve relative to its own worktree unconditionally, not just advisory text) the [#1319](https://github.com/mattsears18/shipyard/issues/1319) issue's suggestion 2 describes.
