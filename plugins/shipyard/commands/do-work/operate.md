...
### Backend reachable but unauthenticated — sign-in wall detection and cookie-import failure semantics ([#1401](https://github.com/mattsears18/shipyard/issues/1401))

**A different case than [Degradation](#degradation--no-backend-reachable) above.** Degradation covers no backend being reachable *at all*. This covers the far more common case: a backend connects cleanly, returns HTTP 200, and still lands on a sign-in wall instead of the target surface — because it's running its own isolated profile (the headless backend's default state) rather than the user's real, logged-in one. Don't collapse the two: a hand-back that reads "no backend available" when the real state is "backend up, not signed in" sends the next session down the wrong recovery path, and this exact confusion cost four consecutive sessions on `mattsears18/lightwork` before the cause was pinned down.

**Detection: don't trust a 200 or a rendered page as proof of an authenticated session.** `shipyard:auditing-authenticated-surfaces`'s [rule 4](../../skills/auditing-authenticated-surfaces/SKILL.md#4-assert-on-a-protected-route-not-) already documents the reliable check in full — assert on a route that redirects to a sign-in host when unauthenticated, never on `/` or a bare 200 — and this operator layer hits the identical wall that skill was written for (a headless backend running its own profile). Reuse that technique rather than re-deriving it here.

**Remedy: cookie import — and its two silent-failure causes.** When the headless backend (tier 1) lands on a sign-in wall for a site the user *is* authenticated to in their real browser, `/setup-browser-cookies` (item 1 above) is the documented remedy. It can fail **silently** — a clean-looking result, zero cookies imported, no error — for two specific reasons:

1. **The source browser is running.** A live Chrome process holds a lock on its own `Cookies` SQLite DB and encrypts cookie values against the OS login keychain; a non-interactive import against a running Chrome can silently return zero rather than erroring.
2. **Wrong profile directory.** The import targets a specific profile path; a stale or mismatched path returns zero matches with no indication the path itself was the problem.

**`Imported 0 cookies` is a FAILURE, not a clean no-op.** Read literally it's indistinguishable from "you're already logged out everywhere" — that's the trap: the account can be very much signed in in the source browser while the import still returns zero for either cause above. Treat a zero-cookie-import result the same way [the exhaustion checklist's item 3](./operate/03-error-handling-and-safety.md#exhaustion-checklist--before-declaring-a-console-unreachable-998) already treats any zero-result helper — inconclusive, never evidence of absence — and check the two causes above before falling through to the next backend tier.

**Hand-back shape.** If both causes are ruled out (or can't be verified from within the session) and the surface still can't be reached, hand back naming the **specific one-line local action** that unblocks it — e.g. *"quit Chrome fully, or run `/setup-browser-cookies` once from an interactive shell so the keychain prompt can be answered, then re-run"* — never a generic "operator work blocked" or "no backend available." Record which cause applied once, in the issue comment, so a repeat hit is recognized rather than re-derived from scratch next session.
