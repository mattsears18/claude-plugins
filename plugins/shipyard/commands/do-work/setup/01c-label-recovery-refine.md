...
| **Commits ahead, but content already landed** ([#1517](https://github.com/mattsears18/shipyard/issues/1517)) | `git -C <path> diff --quiet origin/<default-branch> HEAD` exits `0`, **or** the issue number parsed from `do-work/issue-<N>` resolves to a `CLOSED` issue with a non-empty `closedByPullRequestsReferences` | **No PR.** `git worktree remove <path>` (non-force only) → `gh issue edit <N> --remove-assignee @me`; the local branch is deleted only on the empty-diff signal. `already_landed_count++`. See [below](#already-landed-pre-checks-1517). |
