...
**A shell redirect target outside the worktree is refused too — and the message misattributes it to git ([#1325](https://github.com/mattsears18/shipyard/issues/1325)).** This is a **third, distinct** post-relocation refusal trigger — alongside the two documented in [Claude Code's command-shape check](https://code.claude.com/docs/en/worktrees#how-claude-code-enforces-isolation) (a braced `${VAR}` expansion; argument-position `$(cmd)` substitution) and [`dont.md`](../dont.md#post-relocation-bash-blocks-must-be-plain-single-purpose-commands-1277)'s compound-block rule (loops, pipes, `if`/`case` wrappers spanning multiple `gh`/`git` calls). A **plain, single-statement, non-git command** — no braces, no substitution, no loop — is still refused post-relocation when its only shell redirect (`>`, `>>`, `2>`) targets a path outside this worktree, e.g. a scratch dir under `$CLAUDE_JOB_DIR/tmp/` or `/tmp/`:
