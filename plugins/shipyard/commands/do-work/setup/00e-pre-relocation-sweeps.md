...
5. **[Step 3c](01c-label-recovery-refine.md#3c-orphan-worktree-triage)** — the orphan `do-work/*` branch triage, via `worktree-reap.sh triage-orphan-branches` ([#1365](https://github.com/mattsears18/shipyard/issues/1365)). Run it here **synchronously (foreground), not backgrounded** — a foreground call keeps the pre-relocation ordering trivially verifiable rather than depending on a background job's survival across the later `EnterWorktree` call.
...
The caller must resolve `--repo-root` in its own, separate `Bash` call rather than in the same block as the sweep invocation — a data-flow-dependent refusal risk documented in [Claude Code's command-shape check](https://code.claude.com/docs/en/worktrees#how-claude-code-enforces-isolation).
