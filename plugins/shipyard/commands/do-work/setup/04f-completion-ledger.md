...
This ledger does not implement the `unfiltered_open_count` / `me_assigned_open` invariant-line tokens (companion issue [#1246](https://github.com/mattsears18/shipyard/issues/1246)) or make the eligibility filter itself executable rather than prose-re-derived at three call sites (companion issue [#1247](https://github.com/mattsears18/shipyard/issues/1247)). Once #1246 lands, the ledger's header line can read those tokens directly instead of re-deriving `<total>` from the fetch; once #1247 lands, the bucket-assignment logic above can call the shared filter instead of re-testing each signal inline. Neither dependency blocks this ledger from catching the failure mode today — it is a reporting and gating layer on top of whatever the filter currently does, not a replacement for it.
