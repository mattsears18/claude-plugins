...
**Invocation — one plain command ([#1398](https://github.com/mattsears18/shipyard/issues/1398)).** [`scripts/classify-backlog.sh`](../../../scripts/classify-backlog.sh)'s `run` subcommand absorbs the whole input-gathering + classify sequence — the seven config reads (including `backlog.someday_milestone` and `backlog.someday_recheck_days`, issues [#1406](https://github.com/mattsears18/shipyard/issues/1406)/[#1422](https://github.com/mattsears18/shipyard/issues/1422)), both live-network precomputed sets, the conditional probe-verdicts call, and the someday-recheck marker write — so the orchestrator's own call is just script-invocation plus the handful of inputs only the caller can supply (the `$CLAUDE_PLUGIN_ROOT`/`$SHIPYARD_REPO_ROOT` pins, `$ME_LOGIN`, and the repo-scoped CSVs from earlier steps). Previously this section inlined the whole sequence as a single large `Bash` block — the spec's own prose called out that it genuinely needed to stay one call (the intermediate variables don't survive between separate `Bash` calls) — but pasting it as written was refused verbatim by the worktree-isolation guard ("this command is too complex to verify that it stays inside the worktree"), forcing every orchestrator session to improvise its own workaround. Extracting it mirrors the `stale-check-refresh.sh` / `next-available-version.sh` / `pre-dispatch-branch-reap.sh` / `concurrent-session-guard.sh` precedent (#1289): the classify-backlog.sh header is the non-normative description of what it does internally; this file's own copy of that description is intentionally brief — see the script's header for the full contract (`--repo`/`--me`/`--trusted-authors`/`--issues-file` required, `--peer-claimed`/`--prioritize-label`/`--out` optional).

**Someday-recheck marker write (issue [#1422](https://github.com/mattsears18/shipyard/issues/1422), follow-up to #1406) rides inside this same call — no separate step needed.** After `classify-backlog.sh run` produces the NDJSON, it pipes that same NDJSON into `backlog-filter.sh someday-recheck-write` (best-effort, never fails the whole call) — for every `drop:someday-milestone` line tagged `someday_recheck_action: "first-park"` or `"cheap-reset"`, this writes or refreshes a `<!-- do-work-someday-recheck: YYYY-MM-DD -->` body marker dated `today + backlog.someday_recheck_days` days. This is the mechanical half of the slow-cadence re-scope: no scope-agent runs, no `raw_backlog` membership changes, just a cheap metadata refresh so the SAME issue isn't re-evaluated again until the cadence next elapses. A `someday_recheck_action: "not-due"` line (marker still future) needs no write; a `someday_recheck_action`-free `eligible` line is the ESCALATE case (see below) and is already in `raw_backlog` by construction. See `backlog-filter.sh`'s own header comment for `someday_recheck_state()`'s full four-state contract, and [`do-work-RATIONALE.md`](../../do-work-RATIONALE.md) for why this reuses `do-work-blocked-until`'s marker SHAPE under a distinct marker name rather than a fourth cadence mechanism.

**A cadence-elapsed, changed-since-write someday-parked issue is not dropped at all this pass — it's a plain `eligible` line in `raw_backlog`,** exactly like any other candidate, for exactly ONE real scope-agent pass (`someday_recheck_state`'s `"escalate"` outcome). Whatever the scope agent concludes — ready, or a defer of any class — is handled by the ordinary [scope pre-flight](06-scope-preflight.md) machinery with one addition: [`06c-scope-handling-ui.md` step 4d](06c-scope-handling-ui.md#handling-each-returned-entry-fires-as-each-background-agent-completes) resets the someday-recheck clock whenever the escalated issue's own defer conclusion lands, so the cadence doesn't collapse back to "escalate every session" the instant it fires once.
