...
- **Route `needs-triage` issues to `investigate_candidates` (gated on `triage.investigate_dispatch`).** Trusted-author issues carrying `needs-triage` survive the author-login gate above and reach this point. Check the config key:
  ...
  **`investigate_dispatch == "false"` is the only configuration under which `needs-triage` is a human item** ([#1077](https://github.com/mattsears18/shipyard/issues/1077)) — under the default, a trusted-author `needs-triage` issue is autonomously dispatched via investigate mode and no human action is required, so [`01b`'s bucket-5 scoring](01b-backlog-overview.md#2-backlog-overview) and `/my-turn`'s survey both key off this same config value rather than independently guessing when the label is actionable by a human.
