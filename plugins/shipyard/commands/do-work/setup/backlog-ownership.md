...
This is a **structural** check, not a full semantic-equivalence check — it does not compare `01b`'s prose description of a bucket's *disposition* against `04`'s actual runtime behavior word-for-word. Bucket 5 is the known example: `01b` and `04` currently describe `needs-triage`'s handling in language that reads differently even though both cite the same underlying behavior; reconciling that wording is [#1077](https://github.com/mattsears18/shipyard/issues/1077)'s scope, not this table's. This table records `04`'s actual behavior as the Owner-relevant fact (bucket 5 IS `/do-work`'s, per the corrected tally in #1076) while leaving the prose-reconciliation itself to #1077.
...
| 5 | Needs triage | carries `needs-triage` | trusted-author → routed to `investigate_candidates`; untrusted-author → dropped (folds into 0.5) | **`/do-work`** | `01b`'s prose vs `04`'s actual routing reads differently — tracked in #1077 |
...
- A row whose Notes cites an open tracking issue (#1077 / #1078 / #1079) is a **known, separately-scoped** gap or overlap, not something this table or its coverage check papers over. Resolving one of those issues means updating that row's Owner/Notes here, not re-deriving the table from scratch.
- Issue [#1076](https://github.com/mattsears18/shipyard/issues/1076) is what gave buckets **1** and **4** their first real, stated owner. The three higher-value cells (0.5, 5, 8) were deliberately left as open, separately-tracked gaps rather than folded into that same PR — see the issue's own scope note.
