...
- **`<s>` (someday-parked, [#1406](https://github.com/mattsears18/shipyard/issues/1406)) carries no date annotation, unlike `<t>` above — deliberately.** `<s>` is simply the count of `classify` verdicts `{"verdict":"drop","reason":"someday-milestone"}`, rendered plainly as `<s> someday-parked` with no "(next clears ...)" suffix: this park has no calendar and no probe, so there is no knowable-moment to report — see `scripts/backlog-filter.sh`'s header comment for why this is a genuinely different admission question than `<t>`'s. Same non-zero-only clause convention: `<s> == 0` (the default on any repo that has not set `backlog.someday_milestone`) contributes nothing to the census line at all.
