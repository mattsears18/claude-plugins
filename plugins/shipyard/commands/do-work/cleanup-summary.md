...
0. **Record why the session ended, from data already in hand — non-fatal ([#1252](https://github.com/mattsears18/shipyard/issues/1252)).** Runs first, before any reaping starts. Three of six recent sessions terminated with a non-empty `in_flight` and no record of why — the orchestrator process ended before it ever reached this point, indistinguishable after the fact from a clean exit. Stamping a reason here — even though it can't help THAT class of failure directly — means every session that *does* reach cleanup leaves an unambiguous trail, and a predecessor that *doesn't* reach it is exactly what's missing: the next session's orphan sweep (below) now reads a still-null `.session_end` on a dead-PID file as the signal itself.
   ...
   **`completed` is not itself the whole story — carry the ledger's `empty`/`fully-gated` terminal-state naming into `--detail` too ([#1358](https://github.com/mattsears18/shipyard/issues/1358)).** A `completed` reason covers two opposite health signals that read identically from the reason token alone: `<total_open>` == 0 (nothing is open — `empty`) and `<total_open>` > 0 with every open issue parked in one of the ledger's buckets 1–9 (nothing is dispatchable, but plenty remains open and gated — `fully-gated`). Compute `TERMINAL_STATE` from the same `<total_open>` the ledger's [termination-assertion build](./drain.md#termination-assertion) already produced (`empty` when it's 0, `fully-gated` when it's > 0 — `n/a` for `bounded-exit`/`user-stop`, since the distinction only applies to a genuine `completed` exit) and fold it into `--detail`:
   ...
   "$CLAUDE_PLUGIN_ROOT/scripts/session-state.sh" record-session-end \
     ...
     --detail "drain exited via <drain-exit-reason>; session_prs merged=<m> blocked:ci=<c> rebase-blocked=<r> pending=<p>; ledger dispatchable=<bucket10> unaccounted=<bucket11>; terminal-state=<empty|fully-gated|n/a> (<total_open> open)" \
   ...
   **Never let `fully-gated` read as an unqualified success.** The [End-of-session summary](#end-of-session-summary)'s `Session end` line renders `<detail>` verbatim, so `terminal-state=fully-gated (<total_open> open)` always carries the open count alongside the `completed` reason — a reader can't mistake it for "nothing left, repo caught up" the way a bare `completed` would read. The bucket breakdown printed immediately above that line already names *which* buckets are holding those `<total_open>` issues open (human-gated, time-gated, operator queue, …) — this token is what tells the reader to look at that table instead of skimming past a "completed" line as good news.
...
7.6. **Persist this session's operator denials to the durable cross-session registry — before the session state file is removed ([#1363](https://github.com/mattsears18/shipyard/issues/1363)).** The session-local `operator_denials` struct (see [`orchestrator-state-reference.md`](./orchestrator-state-reference.md)) is already surfaced within this session's own summary (the `Operator denied (#746)` block below), but it's discarded the moment step 8 removes the session-state file — so a denial's only durable trace, absent this step, is whatever got hand-written into a PR/issue comment. A denial is not transient session noise: it's a standing statement about what this harness will and will not let an agent do on a given surface, and the next session that hits the same denial shouldn't have to rediscover it from scratch. Same before-the-retirement ordering as step 7's cost-ledger flush.
   ...
   **Gate on `operator_denial_registry.enabled` (default `false`) — opt-in, same posture as `flake_registry`:**
   ...
   "$CLAUDE_PLUGIN_ROOT/scripts/operator-denial-registry.sh" record \
     ...
     --kind "<entry.kind>" \
     --target "<entry.target>" \
     --denial-text "<entry.denial_text>" \
     --outcome "<entry.outcome>" \
     --session "<session-id>" \
     --at "<entry.denied_at>"
   ```

   `record` is append-only and non-blocking on failure — same don't-gate-exit posture as every other cleanup step; a failed append is a lost data point, not a reason to stall session exit. See [`scripts/operator-denial-registry.sh`](../../scripts/operator-denial-registry.sh)'s own header for the on-disk shape, the local-only privacy posture (same as `flake-registry.jsonl`, but off by default because a `target` can be a specific admin-console URL — more identifying than a CI test name), and the `report` subcommand a future session (or a human) uses to see whether a given `(repo, kind, target)` denial is a one-off or a repeated, standing constraint.
...
8.5. **Milestone roadmap sweep (end-of-loop, opt-in — [#1243](https://github.com/mattsears18/shipyard/issues/1243)).** A `/do-work` loop is precisely the event that makes a roadmap stale — it closes issues in bulk, opens PRs, and files new ones (auditor findings, deferred slices, decomposed epics), all of which arrive with no milestone. Run this after the reap/flush/cleanup steps above (so `shipped_count` and `filed_count` are already known) and before the [End-of-session summary](#end-of-session-summary) renders, so its tallies can fold straight into that summary instead of producing a second report.
   ...
   **Gate — both keys, not just `enabled`:**
   ...
   When either value is anything other than `true`, this step is a complete no-op: no reads, no writes, no summary line, `roadmap_sweep_ran` stays `false`. This mirrors every other `milestones.*` consumer's inertness contract (see [`do-work/dont.md`'s milestones bullets](./dont.md)).

   **Skip cleanly when the loop did nothing.** When both gate values are `true` but this session's `shipped_count + filed_count == 0` (a drain-only or no-op loop — nothing closed this session, nothing filed), also skip — there is no new unmilestoned artifact for a loop-end sweep to catch up on, and running it anyway burns a full backlog read for zero marginal benefit. `roadmap_sweep_ran` stays `false`; no summary line.

   **Run — steps 1–6 of the skill's own procedure, never step 7.** Otherwise, follow the [`shipyard:update-roadmap`](../../skills/update-roadmap/SKILL.md) skill's [Procedure](../../skills/update-roadmap/SKILL.md#procedure) steps 1–6 in full: gate check, fetch current state, cold-start check, unmilestoned sweep, dependency sweep, honesty checks. **Skip the skill's own step 7 (Report)** — do not post a separate GitHub comment and do not print a standalone summary; that would be a second report describing the same run. Instead record the tallies for the summary below: `roadmap_applied_count` (issues assigned a milestone + phases/fallback created + dependencies recorded — all mechanical, all applied) and `roadmap_proposed_count` (phase proposals + inferred-dependency proposals + honesty-check findings — all propose-only, none applied). Set `roadmap_sweep_ran=true`.

   This step runs in the **orchestrator's own turn** — never delegate it to a dispatched worker. See `shipyard:worker-preamble`'s [`milestone-prohibition.md`](../../skills/worker-preamble/milestone-prohibition.md) fragment and the skill's own "Who runs this" section for why a worker, scoped to a single issue in an isolated worktree, cannot safely take this on.

   **Non-fatal — catch, don't gate.** Any failure inside the run above (a `gh` API error, a schema-validation miss, a malformed milestone title, a rate limit) is caught and folded into `roadmap_sweep_error` as a one-line reason — it must NOT change the loop's exit status, must NOT block or delay the reap/flush/cleanup steps above (they've already run by this point), and must NOT block the [End-of-session summary](#end-of-session-summary) below. This is hygiene, not a gate: a roadmap sweep failure is a footnote in the summary, never a session-ending error.

8.6. **Detect plugin-root drift during the session ([#1304](https://github.com/mattsears18/shipyard/issues/1304)).** `CLAUDE_PLUGIN_ROOT` is resolved once, at [step 0.5](./setup/00-config-worktree.md#05-move-into-the-orchestrators-worktree), and stashed for the rest of the session — every later block reads that stash rather than re-resolving. On a **consumer install** (the `installed_plugins.json`-resolved layer), the harness's own plugin manager can update the installed version mid-session with nothing to notice: the orchestrator keeps invoking `scripts/*.sh` from whatever version it stashed at startup, while the `shipyard:worker-preamble` skill text a dispatched worker reads resolves live against the *current* install — silently splitting "which spec ran" from "which scripts ran" for the rest of the session. This step doesn't prevent that split; it re-resolves fresh, one last time, so a session that hit it says so instead of leaving the drift invisible.

   Cheap and read-only — no re-export of `CLAUDE_PLUGIN_ROOT` needed, this step never invokes a script from it:

   ```bash
   SHIPYARD_PLUGIN_ROOT_START="$(cat .shipyard-plugin-root 2>/dev/null)"
   SHIPYARD_PLUGIN_ROOT_VERSION_START="$(cat .shipyard-plugin-root-version 2>/dev/null)"
   [ -z "$SHIPYARD_PLUGIN_ROOT_VERSION_START" ] && SHIPYARD_PLUGIN_ROOT_VERSION_START="unknown"
   TOPLEVEL="$(git rev-parse --show-toplevel)"
   SHIPYARD_PLUGIN_ROOT_DRIFT=""
   # The repo-local layer (dogfooding — this checkout IS the plugin source)
   # can't drift mid-session on its own: it's a fixed git ref for the whole
   # run, not the harness-managed install. Drift is only possible when the
   # session originally resolved the installed_plugins.json layer.
   if [ "$SHIPYARD_PLUGIN_ROOT_START" != "$TOPLEVEL/plugins/shipyard" ]; then
     INSTALLED_PATH_NOW=$(jq -r '.plugins["shipyard@shipyard"][0].installPath // empty' "$HOME/.claude/plugins/installed_plugins.json" 2>/dev/null)
     if [ -n "$INSTALLED_PATH_NOW" ] && [ -d "$INSTALLED_PATH_NOW/scripts" ] && [ "$INSTALLED_PATH_NOW" != "$SHIPYARD_PLUGIN_ROOT_START" ]; then
       SHIPYARD_PLUGIN_ROOT_VERSION_NOW=$(jq -r '.version // empty' "$INSTALLED_PATH_NOW/.claude-plugin/plugin.json" 2>/dev/null)
       [ -z "$SHIPYARD_PLUGIN_ROOT_VERSION_NOW" ] && SHIPYARD_PLUGIN_ROOT_VERSION_NOW="unknown"
       SHIPYARD_PLUGIN_ROOT_DRIFT="$SHIPYARD_PLUGIN_ROOT_VERSION_START -> $SHIPYARD_PLUGIN_ROOT_VERSION_NOW"
     fi
   fi
   echo "plugin-root-drift: ${SHIPYARD_PLUGIN_ROOT_DRIFT:-none}"
   ```

   **Read the printed line back**, same discipline as step 5.5's `unreaped-worktrees:` echo — a shell variable set inside this call doesn't survive into the [End-of-session summary](#end-of-session-summary) render below. `SHIPYARD_PLUGIN_ROOT_DRIFT` empty (the overwhelming common case: no consumer-install session hit an update, or the session ran the dogfooding repo-local layer) → the summary's `Plugin root moved:` line is omitted entirely. Non-empty → this session's helper scripts and orchestrator spec ran against `<start>` for their entire duration while a newer `<now>` install landed underneath it. This is advisory only, reported after the fact — it does NOT re-point `CLAUDE_PLUGIN_ROOT` mid-session (that would risk a live spec/script-version mismatch worse than the stale-but-consistent one this session already ran on) and does NOT retry anything; it tells the user to re-run soon to pick up the newer install.

8.7. **Report how far the orchestrator worktree's spec copy drifted during the session ([#1486](https://github.com/mattsears18/shipyard/issues/1486)).** The sibling of step 8.6, one layer over: 8.6 asks whether the **installed plugin** moved underneath a consumer-install session; this step asks how far **`origin/<default-branch>` moved away from the spec copy this session was actually executing.** The orchestrator worktree is created once at [setup step 0.5](./setup/00-config-worktree.md#05-move-into-the-orchestrators-worktree) and deliberately never refreshed — correct behavior, and explicitly not something this step changes — but on a dogfooding session that ships changes to `/do-work`'s own spec, the executing copy drifts arbitrarily far with nothing anywhere saying so. Note 8.6 cannot cover this case even in principle: it short-circuits the repo-local layer on the grounds that a fixed git ref "can't drift mid-session on its own," which is true of the *ref* and precisely the problem for the *spec*.

   **Skip entirely on the installed-plugin layer** — if session-local `SHIPYARD_SPEC_DRIFT` is `n/a` (set at step 0.5), there is nothing to report and no commands to run; the `Spec drift:` line is omitted. Otherwise re-measure once, so the summary reports the drift at session *end* rather than at whichever step-D refresh happened to fire last. `<default-branch>` is the resolved literal, substituted by the orchestrator:
   ...
   Read that integer back as `SPEC_DRIFT_END` (empty or non-zero exit → `unknown`). **Zero → omit the line** — a session whose spec copy is still current has nothing worth a summary row.

   Non-zero: name the spec-touching subset, because that is the half an operator can act on. One plain local command, no `gh` calls:

   ```bash
   git log --oneline "HEAD..origin/<default-branch>" -- plugins/shipyard/commands/do-work/
   ```

   Each squash-merged subject carries its PR number as a trailing `(#M)`. Read those numbers out of the printed subjects yourself (no pipe needed — you already have the output), and **intersect them with `session_prs`**: the intersection is the set of PRs *this session shipped* that changed the very spec it was running, which is the case where "the fix I shipped didn't apply to me" is worth an operator's attention. An empty intersection is normal and still reports the raw count — it just means the drift came from elsewhere.

   Advisory only, exactly like 8.6: it does **not** refresh, reset, or re-read the worktree, and does not retry anything. Mutating the spec under a running loop is a larger hazard than the one this line reports; if a refresh is ever wanted it is an explicit operator action.
...
Backlog: 12 open — 0 workable · 5 time-gated (next clears 2026-08-20) · 1 someday-parked · 2 blocked (body reference) · 2 awaiting human review · 2 agent-console (operator action)

Bucket                                       Count   Issues
───────────────────────────────────────────  ─────   ──────────────────────────────────────────────
Workable (remaining after session)               2   #<a>, #<b>   — OR reason text if 0
⚠️ Unaccounted (filter defect, #1250)            1   #<w>         — row present only when count > 0
...
Discussion                                       1   #G
Won't fix                                        1   #X
Someday-parked (#1406)                           1   #Y (milestone: 6 · Someday)  — row present only when backlog.someday_milestone is set
...
The full shape, **zero-row mode** (everything closed this session — clean board; only reachable when `Unaccounted == 0` too — see the gate above):
...
Config (fail-open reads, #1388): <CONFIG_FAILURE_ROWS> schema-validation failure(s) recorded after step 0.4 — <FILE>: <REJECTED>; every fail-open get at those call sites silently used built-in defaults
Config-named label(s) missing (#1359): <SHIPYARD_LABEL_CONFIG_MISMATCH> — routing that depends on them is silently dead until the label is created or the config key repointed
Plugin root: <SHIPYARD_PLUGIN_ROOT_STALE> — run git pull --ff-only before trusting this session's spec-based conclusions (#907)
Primary checkout self-healed (#1386): <SHIPYARD_PRIMARY_SELF_HEALED> — step 0.41's gate fast-forwarded the primary checkout and restarted setup from step 0.3; the session ran the CURRENT spec, not the stale one
Plugin root moved: <SHIPYARD_PLUGIN_ROOT_DRIFT> — this session ran the whole way through on the earlier version; re-run soon to pick up the newer install (#1304)
Skill cache: <SHIPYARD_SKILL_CACHE_STALE> — dispatched workers loaded shipyard:worker-preamble by name from this stale cache all session; run /shipyard:update then /reload-plugins (#1319)
Spec drift (#1486): <SPEC_DRIFT_END> commit(s) — the orchestrator worktree executed a spec copy this far behind origin/<branch> by session end; <SPEC_DRIFT_SPEC_TOUCHING> of those touched commands/do-work/**<SPEC_DRIFT_OWN_PRS_CLAUSE>. The pin is intentional; no action needed unless you want the newer spec, which means a fresh session.
Pre-relocation step(s) caught (#1351): <SHIPYARD_PRERELOC_STEP_CAUGHT> — found and run at step 0.42, ahead of their canonical position, because this session's primary checkout was stale enough to predate them
Pre-relocation step(s) skipped (#1351): <SHIPYARD_PRERELOC_STEP_MISSED> — discovered only at step 0.6, after EnterWorktree already ran; too late to execute in the documented pre-relocation position this session
...
Queued behind in-flight PRs (#1426): <Qp> (#P — blocking PR(s) #A, #B, #Q — blocking PR #C, ...)
...
Environmental pauses (#1402): <paused> pause(s) totaling <total_paused_minutes>m — <resumed> resumed automatically, <expired> expired to hand-back
Fix-checks green verification (#1383): <model1>: <confirmed1>/<confirmed1+downgraded1> confirmed (<downgraded1> downgraded), <model2>: <confirmed2>/<confirmed2+downgraded2> confirmed (<downgraded2> downgraded), ... — escalated to sonnet: <fix_checks_false_green_prs count> PR(s) (#<a>, #<b>, ...)
...
Roadmap sweep (#1243): <roadmap_applied_count> applied (milestones assigned / phases or fallback created / dependencies recorded), <roadmap_proposed_count> proposed — see [roadmap proposal] comments
...
⚠️  CI executor capacity (#1141): self-hosted pool has <ci_pool_total> runner(s); <ci_queued_at_exit> run(s) queued at session end (~<ci_queue_ratio>x pool capacity) with <F> PR(s) still open — CI capacity, not correctness, is likely why they haven't landed yet. Consider a lower --concurrency next run, or routing CI-cheap (docs-only) work to its own session.
```
...
- **`Someday-parked` row ([#1406](https://github.com/mattsears18/shipyard/issues/1406)).** Sourced from the completion ledger's fractional bucket 7.5 (see [`setup/04f-completion-ledger.md`](./setup/04f-completion-ledger.md#the-bucket-taxonomy)) — issues whose `classify` verdict was `{"verdict":"drop","reason":"someday-milestone"}`. Same non-zero-only row rule as every other bucket: omit entirely when the count is 0, which is every session on a repo that leaves `backlog.someday_milestone` at its default empty string (the overwhelming common case — the clause is off by construction, see the config key's own schema description). This is the row that satisfies "surface the count so a growing park pile stays visible instead of becoming a quiet dumping ground" — the same reasoning `tracking-unjustified` gets its own sub-line for rather than folding into a flat count.
...
```
Backlog: <total> open — <W> workable · <t> time-gated (next clears <YYYY-MM-DD>) · <s> someday-parked · <b> blocked (body reference) · <h> awaiting human review · <o> agent-console (operator action) · ...
```
...
- **Bucket names are lifted verbatim from [`backlog-ownership.md`](./setup/backlog-ownership.md#ownership-table)'s Bucket column — never restated independently.** `workable` ← bucket 8 "Workable"; `blocked (body reference)` and `time-gated` ← the two sub-conditions bucket 7 "Blocked (body reference / time-gate)" names in its own row title (the row bundles both conditions into one bucket for dispatch purposes, but the census still needs them as separate clauses per the next bullet, so both terms come from words already present in that one row's own name — not a new taxonomy); `awaiting human review` ← bucket 5.5 "Awaiting human review"; `agent-console (operator action)` ← bucket 5.6 "Agent-console (operator action)"; `needs triage` ← bucket 5 "Needs triage"; `won't fix` ← bucket 3 "Won't fix"; `discussion` ← bucket 4 "Discussion"; `in flight` ← bucket 2 "In flight"; `untrusted author` ← bucket 0.5 "Untrusted author". `someday-parked` is the one term with no `backlog-ownership.md` row to lift from — that table predates [#1406](https://github.com/mattsears18/shipyard/issues/1406) — so it is named directly from `classify`'s own `someday-milestone` reason string (dropping the redundant "-milestone" suffix, since the milestone name itself renders alongside it per the bucket-table row above). Map classify's machine-readable `reason` strings onto these names via the same reasons [`04-backlog-divert.md`'s Invocation block](./setup/04-backlog-divert.md#4-fetch--rank-the-backlog) and [`backlog-filter.sh`](../../scripts/backlog-filter.sh) already emit (`time-gated`, `blocked-by-open-issue`, `needs-human-review`, `operator`/`agent-console`, `someday-milestone`, etc.) — the mapping is a rendering choice, not a second definition of what each bucket means.
...
- **`<s>` (someday-parked, [#1406](https://github.com/mattsears18/shipyard/issues/1406)) carries no date annotation, unlike `<t>` above — deliberately.** `<s>` is simply the count of `classify` verdicts `{"verdict":"drop","reason":"someday-milestone"}`, rendered plainly as `<s> someday-parked` with no "(next clears ...)" suffix: this park has no probe and — unlike `<t>`'s calendar, which is a direct dispatch-eligibility gate — its own internal `backlog.someday_recheck_days` cadence ([#1422](https://github.com/mattsears18/shipyard/issues/1422)) is not a "next clears" moment either, since elapsing it does not by itself make the issue dispatch-eligible; it only triggers a cheap marker refresh or (rarely) one scope-agent pass, and the PARK itself only ever clears when a human moves the issue out of the milestone. So there remains no knowable "becomes workable" moment to report — see `scripts/backlog-filter.sh`'s header comment for why this is a genuinely different admission question than `<t>`'s. Same non-zero-only clause convention: `<s> == 0` (the default on any repo that has not set `backlog.someday_milestone`) contributes nothing to the census line at all.
...
- `Config (fail-open reads):` line ([#1388](https://github.com/mattsears18/shipyard/issues/1388)): the belt-and-suspenders companion to `Config:` above, covering every schema failure step 0.4's `load` was *not* the call to hit. Step 0.4 is the only config read that inspects the loader's exit code; every other consumer — [`resolve-dispatch-model.sh`](../../scripts/resolve-dispatch-model.sh), [`flake-enforce.sh`](../../scripts/flake-enforce.sh), and the worker specs' `shipyard-config.sh get ... 2>/dev/null || echo <default>` reads — is deliberately **fail-open**, returning an empty string with exit 0 and no output. That posture is correct for availability and is unchanged, but it makes a schema-invalid config indistinguishable from "this repo sets no override for that key" at every one of those call sites. [`shipyard-config.sh`](../../scripts/shipyard-config.sh) therefore appends each distinct failure to a session-local marker file in the orchestrator worktree, alongside the `.shipyard-plugin-root` / `.shipyard-primary-root` stashes. Read it with one plain command:

  ```bash
  cat .shipyard-config-schema-failure 2>/dev/null
  ```

  **Omit the line entirely** when the file is absent or empty — silence is the default, and the overwhelming-common case is a clean config load. When non-empty, each row is `<ISO8601-UTC>\t<config file>\t<rejected fields>` (the rejected-fields column uses the same `; `-joined shape as `SHIPYARD_CONFIG_SCHEMA_FAILURE`). Substitute `<CONFIG_FAILURE_ROWS>` with the row count, and `<FILE>` / `<REJECTED>` with the **last** row's file and rejected-fields columns (rows are deduplicated on `(file, fields)`, so a count above 1 means genuinely distinct failures — usually the repo layer and the gitignored `.shipyard/config.local.json` layer failing for different reasons — not the same failure re-read on every turn).

  **Lifetime and scope.** The marker is written only when `.shipyard-plugin-root` exists in the cwd — the orchestrator worktree's own signature. That worktree is created per session and reaped at session end, so the record is session-scoped by construction with no clearing step and no cross-session bleed; and a dispatched worker's own `get` can never drop an untracked file into a worktree whose diff becomes a PR. Orchestrator-side reads are what the record is for anyway, and a worker reading the same broken config hits the same failure the orchestrator already recorded. **This line frequently prints alongside `Config:` above** (the same broken config trips both step 0.4's `load` and every later `get`) — that is expected, not a duplicate: `Config:` says the session started degraded, this line says the degrade kept biting at call sites that never checked. It also prints **alone** in the case #1388 exists for — a config that validated fine at step 0.4 but was edited (or whose gitignored local override drifted) mid-session.
- `Config-named label(s) missing:` line ([#1359](https://github.com/mattsears18/shipyard/issues/1359)): omit entirely unless the session-local `SHIPYARD_LABEL_CONFIG_MISMATCH` was set at [step 1.75](../do-work/setup/01e-verify-config-labels.md#175-verify-config-named-labels-exist-in-the-target-repo-1359) (i.e. `scripts/verify-config-labels.sh` found — or could not check — one or more `labels.<key>` values from the merged config that don't exist as a label object in the target repo). When set, print the line with `<SHIPYARD_LABEL_CONFIG_MISMATCH>` substituted by the captured `labels.<key>="<value>"` list (or the `could not verify ... (<reason>)` text on an `INDETERMINATE` result, or on a `USAGE_ERROR` result whose retry also failed — [#1492](https://github.com/mattsears18/shipyard/issues/1492)). Silence is the default — a repo whose label set covers every config-named value does NOT print this line. Same re-surfacing pattern as `Config:` above: the check ran once at step 1.75, and this line puts the finding back in front of a user who scrolled past the startup output, since a routing label that was never actually applied would otherwise strand affected issues invisibly for both `/do-work` and `/my-turn`.
...
- `Primary checkout self-healed:` line ([#1386](https://github.com/mattsears18/shipyard/issues/1386)): omit entirely unless the session-local `SHIPYARD_PRIMARY_SELF_HEALED` was set at [step 0.41](../do-work/setup/00-config-worktree.md#041-staleness-gate--self-heal-the-primary-checkout-or-refuse-to-run-1386) (i.e. the staleness gate found a stale-but-clean primary on the default branch, fast-forwarded it, and restarted setup from step 0.3). When set, print the line with `<SHIPYARD_PRIMARY_SELF_HEALED>` substituted by the captured `<head_before> -> <head_after> (<N> commit(s))` detail. Silence is the default. **This is a success line, and it is mutually exclusive with `Plugin root:` above in practice** — a healed session re-runs step 0.4 against the now-fresh checkout, so `SHIPYARD_PLUGIN_ROOT_STALE` comes back unset on the second pass and the stale-warning line is suppressed. It exists because the gate performs the one sanctioned *write* to the user's primary checkout outside A.0.6's branch-leak guard; a write that never appears in the session record is a write the user can't audit.
- `Plugin root moved:` line ([#1304](https://github.com/mattsears18/shipyard/issues/1304)): omit entirely unless the session-local `SHIPYARD_PLUGIN_ROOT_DRIFT` was set by [step 8.6](#end-of-session-cleanup) (i.e. the session originally resolved `CLAUDE_PLUGIN_ROOT` against a consumer install AND a fresh re-resolution at session end found a *different* installed path with a *different* `.claude-plugin/plugin.json` version). When set, print the line with `<SHIPYARD_PLUGIN_ROOT_DRIFT>` substituted by the captured `<start-version> -> <end-version>` detail. Silence is the default — this is a rare mid-session-update case, not something most sessions will ever trip. Distinct from `Plugin root:` above: that line flags a *stale-at-session-start* dogfooding checkout; this one flags a consumer install that moved *underneath* an already-running session.
- `Skill cache:` line ([#1319](https://github.com/mattsears18/shipyard/issues/1319)): omit entirely unless the session-local `SHIPYARD_SKILL_CACHE_STALE` was set at [step 0.4](../do-work/setup/00-config-worktree.md#04-check-the-repo-level-opt-in-shipyardconfigjson) (same dogfooding branch as `Plugin root:` above, but a distinct artifact — the *installed skill cache* `shipyard:worker-preamble` is loaded from by NAME, not the primary checkout `CLAUDE_PLUGIN_ROOT` resolved against). When set, print the line with `<SHIPYARD_SKILL_CACHE_STALE>` substituted by the captured `<cache-version> (installed cache) vs <repo-version> (origin/<branch>)` detail. Silence is the default — a consumer install (where the check doesn't apply) or a session whose installed cache happens to already match `origin/<default-branch>` does NOT print this line. Distinct from `Plugin root:`: that line's remedy is `git pull --ff-only` on the primary checkout; this line's remedy is `/shipyard:update` on the installed plugin cache — the two can be stale independently of each other.
- `Spec drift (#1486):` line ([#1486](https://github.com/mattsears18/shipyard/issues/1486)): **omit entirely** unless [step 8.7](#end-of-session-cleanup) measured a non-zero `SPEC_DRIFT_END` — which additionally requires the session to have run the dogfooding repo-local layer, since 8.7 short-circuits on `SHIPYARD_SPEC_DRIFT == n/a`. Silence is therefore the default for every consumer-install session (all of them) and for any dogfooding session short enough that `origin/<default-branch>` never moved. When it prints: `<SPEC_DRIFT_END>` is the re-measured commit count, `<SPEC_DRIFT_SPEC_TOUCHING>` is how many of those commits touched `plugins/shipyard/commands/do-work/`, and `<SPEC_DRIFT_OWN_PRS_CLAUSE>` is ` (#A, #B — shipped by THIS session, and did not apply to it)` when the intersection with `session_prs` is non-empty, or the empty string when it isn't. That intersection is the line's real payload: it names the PRs where a spec fix this session shipped could not benefit the session that shipped it. Distinct from all three of its neighbours — `Plugin root:` flags a *stale-at-startup primary checkout*, `Plugin root moved:` flags a *consumer install that moved underneath the session*, `Skill cache:` flags a *stale installed skill cache*; this one flags the **orchestrator worktree's own frozen spec copy**, which is the one artifact none of the other three watch. **Deliberately phrased as advisory, not a warning** — the worktree pin is correct behavior and the remedy is "start a fresh session when you want the newer spec," never a mid-session refresh.
- `Pre-relocation step(s) caught:` / `Pre-relocation step(s) skipped:` lines ([#1351](https://github.com/mattsears18/shipyard/issues/1351)): both omit entirely unless the corresponding session-local variable was set. `SHIPYARD_PRERELOC_STEP_CAUGHT` is set at [step 0.42](../do-work/setup/00-config-worktree.md#042-immediate-fresh-re-read-on-staleness-1351) when the immediate git-show-based re-read (fired only on the same staleness condition as `Plugin root:` above) finds a step heading in the 0.40–0.49 range the stale on-disk copy didn't have — the caught case is a **success**: the step still ran, just discovered slightly ahead of its canonical position, so this line is informational, not a warning. `SHIPYARD_PRERELOC_STEP_MISSED` is set at [step 0.6](../do-work/setup/00d-reread.md) (fragment `00d-reread.md`'s advisory) when that same class of step heading is discovered only post-relocation — a genuine miss, since `EnterWorktree` already ran and the step's documented execution window has passed. Print each with the comma-joined step-heading list substituted verbatim. A session can print both (0.42 caught some newly-added steps but its `git show` fetch failed for one, which then only surfaces at 0.6) — they are not mutually exclusive.
- `Session-state mirror degraded (#1302)` block: omit entirely when the session-local `session_state_degraded_since` is `null` for the whole session — silence is the default for the overwhelming-common case (every `session-state.sh` write-class call this session either ran and succeeded, or the field was already cleared by a later recovering write). When `session_state_degraded_since` is non-null (still degraded at session end) or was set-then-cleared mid-session (recovered), print one line naming the onset timestamp and, if recovered, the recovery timestamp and the elapsed window: `⚠️  Session-state mirror degraded from <onset> to <recovery|"session end"> (<duration>) — a session-state.sh write was denied by the classifier or failed; /shipyard:status and this session's own state file may be stale for that window.` This is the end-of-session half of the [#1302](https://github.com/mattsears18/shipyard/issues/1302) fix — the live half is [steady-state.md step E's `state=degraded` invariant-line token](./steady-state.md#e-invariant-line-end-of-every-steady-state-turn), which is required to keep reporting `degraded` every turn the field is set; this block is what makes the *duration* of that degradation legible after the fact, for a session whose transcript a human isn't going to scroll through turn-by-turn. Read `session_state_degraded_since` (and, if the field was cleared mid-session, the recovery timestamp the orchestrator logged at `[session-state] mirror recovered — was degraded since <timestamp>`) from session-local working memory — this field is deliberately never itself written into the (possibly-degraded) session-state file, so it cannot be read back from `session-state.sh read` the way most other lines in this summary are.
...
- `Environmental pauses (#1402)` line: **omit entirely** when `paused_on_environment_stats.paused == 0` — silence is the default for the overwhelming-common case (backpressure never held all the way to a zero-in-flight deadlock this session). When `paused > 0`, print the line so the operator sees the session-wide pause fired, for how long in total, and how it resolved. Read from the session-local `paused_on_environment_stats` counter (see [`orchestrator-state-reference.md`](./orchestrator-state-reference.md)) — never from the live `paused_on_environment` entry itself, which by the time cleanup runs is always already `null` (it's a [termination-registry row](./drain.md#termination-assertion); the session cannot reach cleanup while a pause is still active). A non-zero `expired` count is the one worth a second look — it means the environment did not clear within `paused_on_environment.max_hours` and the corresponding candidates were handed back `needs-human-review` rather than resumed automatically.
- `Fix-checks green verification (#1383)` line: **omit entirely** when the session-local `fix_checks_green_counters` map is empty (no `fix-checks-only` `green`/`noop` return this session — the common case for a session that dispatched no fix-checks work, or whose only fix-checks returns were `pending`/`dirty`/`flake`/`blocked`). When non-empty, render one `<model>: <confirmed>/<total> confirmed (<downgraded> downgraded)` clause per model key present in the map, sorted by dispatch count descending, plus the trailing `escalated to sonnet: <N> PR(s)` clause sourced from `fix_checks_false_green_prs`'s size (omit the trailing clause, not the whole line, when that set is empty — a session can have confirmed greens with zero downgrades). This is measurement, not a verdict — the line exists to make the false-green rate per model tier visible across sessions (feeding a future revisit of [#784](https://github.com/mattsears18/shipyard/issues/784)'s `models.fix_checks_only` tier), not to editorialize about which tier is "worse" from one session's numbers.
...
- `Deferred:` line: omit when `deferred_issues` is empty **after excluding every `blocked-by-in-flight-pr` entry** ([#1426](https://github.com/mattsears18/shipyard/issues/1426) — see the dedicated `Queued behind in-flight PRs:` line below, which those entries render on instead). When the remaining set is non-empty, render one `#N [<defer_reason_class>] — <first sentence of reason>` per entry (truncate at first sentence or 80 chars). The bracketed `defer_reason_class` is one of `external-dependency` / `human-decision-required` / `untrusted-author` / `confirmed-blocker-still-open` / `confirmed-non-shippable-as-single-PR` / `time-gated` ([#298](https://github.com/mattsears18/shipyard/issues/298), [#1165](https://github.com/mattsears18/shipyard/issues/1165)) — never `blocked-by-in-flight-pr`, which this line's `<Df>` count and entry list both exclude by construction. An entry missing this field is a spec violation — when reading from session state for the summary, default to `confirmed-non-shippable-as-single-PR` with a `[shipyard] deferred_issues entry #<N> missing defer_reason_class — defaulted at summary-render time` advisory line above the block. Per [#302](https://github.com/mattsears18/shipyard/issues/302) every entry also carries a non-empty `evidence_pointer` (mechanical citation grounding the class); entries reaching the summary with a missing/empty `evidence_pointer` are spec violations too — emit the same advisory line shape (`[shipyard] deferred_issues entry #<N> missing evidence_pointer — spec violation`) above the block. Full reason is posted as a comment on each issue.
- `Queued behind in-flight PRs (#1426):` line: omit entirely when no `deferred_issues` entry carries `defer_reason_class == "blocked-by-in-flight-pr"` — silence is the default (the overwhelming common case at `--concurrency 1`, where a sibling in-flight PR holding files can't occur, and the common case even at higher concurrency). When `<Qp>` (the count of such entries) is non-zero, render one `#N — blocking PR(s) #A, #B` per entry, reading `#A, #B, ...` from that entry's `blocking_prs` array. **This count is deliberately reported separately from `Deferred:`, not folded into it** — these issues are fully scoped and shippable, merely queued behind a PR the session itself (or a sibling worker) is already landing; folding them into the generic `Deferred:` tally would make a healthy, saturated session that's simply waiting on its own in-flight work read as backlog-stuck. A high `<Qp>` alongside a healthy `Shipped:` count is normal at `--concurrency > 1`; a high `<Qp>` with nothing shipping this session is worth a second look.
- `Roadmap sweep (#1243)` line: **omit entirely** when `roadmap_sweep_ran == false` — silence covers both the disabled-feature case (`milestones.enabled` and/or `milestones.sweep_on_loop_end` not both `true`) and the no-op-loop skip (`shipped_count + filed_count == 0`), matching [step 8.5](./cleanup-summary.md#end-of-session-cleanup)'s own skip conditions. When `roadmap_sweep_ran == true` and `roadmap_sweep_error` is empty, print the line with `<roadmap_applied_count>` and `<roadmap_proposed_count>` as computed by step 8.5. When `roadmap_sweep_error` is non-empty (the sweep ran but failed partway — a `gh` API error, a schema-validation miss), print instead: `Roadmap sweep (#1243): failed — <roadmap_sweep_error> (non-fatal, loop unaffected)`. Either way this line is purely informational — a failure here never changes `Shipped:` / `Blocked:` / any other counter, and never gates the rest of this summary.
...
- `CI executor capacity` block ([#1141](https://github.com/mattsears18/shipyard/issues/1141)): omit entirely unless ALL of the following hold — `.ci_capacity.shape == "self-hosted"` (recorded once at [setup step 1.36](./setup/01-repo-recovery.md#136-detect-ci-executor-pool-capacity-and-clamp-toward-it-1141); silence when `hosted` or `unknown`, since there's either no fixed pool to be capacity-bound by or nothing was readable); a **fresh** re-query of queue depth at session end (`gh run list --repo <owner/repo> --status queued --limit 100 --json databaseId --jq 'length'` — best-effort, and this whole block is skipped, not defaulted to a stale number, if the re-query itself fails) is at least **3×** `.ci_capacity.pool_total`; AND `F` (the `In flight at exit` count from the flat line above) is greater than 0. All three must hold — a session that drained every PR before exiting (`F == 0`) has nothing left waiting on the queue regardless of how deep it got mid-session, so it stays silent. When it fires: `<ci_pool_total>` is `.ci_capacity.pool_total`, `<ci_queued_at_exit>` is the fresh re-query (deliberately NOT `.ci_capacity.queued_at_start` — a multi-hour session's queue depth at the *end* is what explains why PRs are still open, not the depth at the *start*), and `<ci_queue_ratio>` is `<ci_queued_at_exit> / <ci_pool_total>` rounded to one decimal. The 3× multiplier and the `F > 0` gate together are what keep this from firing on an ordinary busy-but-draining session — see [RATIONALE → CI executor pool capacity repro](../do-work-RATIONALE.md#step-136--ci-executor-pool-capacity-repro-1141) for the session that motivated this banner. **This is the "surface the observation" minimum the issue asks for even when [step 1.36](./setup/01-repo-recovery.md#136-detect-ci-executor-pool-capacity-and-clamp-toward-it-1141)'s clamp did NOT fire** — the common trigger is an operator's own **explicit** `--concurrency` (which the clamp never overrides) turning out, in hindsight, to have outrun the pool.
