...
3. **No label disqualifies.** The candidate carries **none** of:
   - `needs-human-review` — signals the issue isn't ready for any worker, inline or dispatched. (`needs-human-review` now subsumes the former `needs-design` design-gate per [#515](https://github.com/mattsears18/shipyard/issues/515). The `needs-refinement` gate was eliminated per [#520](https://github.com/mattsears18/shipyard/issues/520) — `user-feedback` below and the no-headings / `max_body_chars` checks already disqualify the issue shapes the old gate caught.)
