...
| 5 | `investigate_candidates` | Dispatch per [dispatch rule 1.5](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) and return control. | The check that `investigate_candidates` must be empty closes the identical defect `raw_backlog` had before row 4 existed — step 4's own client-side filter drops `needs-triage` issues (they were already routed to `investigate_candidates` at setup, not left in the general fetch), so a routed-but-unworked candidate could never register as net-new either ([#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 2, second application). |
