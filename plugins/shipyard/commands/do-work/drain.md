...
3.5. **`raw_backlog` is empty ([#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 2).** Check from working memory. An issue can sit in `raw_backlog` having been fetched and ranked but never scoped into `ready_issues` — that issue is simultaneously "already known" (subtracted from step 4's net-new diff below) and "not ready" (invisible to step 3), so nothing in the original four-step list ever caught it. If `raw_backlog` is non-empty, trigger [dispatch rule 5](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c)'s background scope-refill (moves entries into `ready_issues`) and return control; this step re-runs once the refill delivers.
...
**A sixth gate: `investigate_candidates` must be empty ([#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 2, second application).** This queue has the identical defect `raw_backlog` had before step 3.5 above — nothing in the original assertion checked it, AND step 4's own client-side filter drops `needs-triage` issues (they were already routed to `investigate_candidates` at setup, not left in the general fetch), so a routed-but-unworked investigate candidate could never register as net-new either. Before declaring termination, `investigate_candidates` must be empty — if non-empty, dispatch it per [dispatch rule 1.5](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) and return control.

**A seventh gate: `divert_queue` must be empty.** Populated by [setup step 4.5](./setup/04-backlog-divert.md#45-divert-checks-main-ci--pr-pileup) and by step D's periodic refresh (main-CI red, PR pileup). If non-empty, dispatch it per [dispatch rule 1](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) and return control.
