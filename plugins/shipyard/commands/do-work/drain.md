...
4. **Fresh-fetch verification.** Run the canonical step-4 backlog query **now** — against the live tracker, not stored state. **Use the same wide-fetch + client-side filter shape that [setup.md step 4](./setup/04-backlog-divert.md#4-fetch--rank-the-backlog) uses** (the previous narrower `-linked:pr` + `-label:...` server-side qualifiers were removed in [#332](https://github.com/mattsears18/shipyard/issues/332) — they silently excluded resumable-work issues like prior-session self-assigns whose linked PR had been closed or never opened, producing premature "backlog empty → drain" handoffs):
   ...
   Then apply the same client-side filter [setup.md step 4](./setup/04-backlog-divert.md#4-fetch--rank-the-backlog) applies — trusted-author check, dispatch-gate labels (`blocked:ci`, `wontfix`, `needs-triage`, `discussion`, `needs-human-review`, `agent-console` — the operator-action gate excluded from code-worker dispatch; drained by the operator phase by default, disabled under `--no-operate` / `--hands-off`), assignee≠@me, `Blocked by #N` still-open, `do-work-blocked-until` still in the future ([#1161](https://github.com/mattsears18/shipyard/issues/1161)), closed-by-@me-authored-healthy-PR — and project the result to a sorted number list for the diff below. Read the client-side filter directly from setup.md's spec; never re-derive it in this file, and never substitute a hand-rolled shorthand for it — that's exactly the bug [#332](https://github.com/mattsears18/shipyard/issues/332) documented, one layer down, and it recurred client-side in a mid-drain ad-hoc query as recently as [#1194](https://github.com/mattsears18/shipyard/issues/1194). `blocked:agent-hard`/legacy `blocked:agent`, `needs-design`, the `needs-decomposition`/`tracking` pair, and `needs-refinement` were eliminated per the binary-backlog migration and no longer appear in this list — see [RATIONALE → Label-lifecycle provenance](../do-work-RATIONALE.md#label-lifecycle-provenance--folded--eliminated-gate-labels) for the full fold/elimination history.

   Stamp `last_fresh_fetch` (the steady-state step E invariant-line token) with the current UTC timestamp the moment this call returns. Stamp `unfiltered_open_count` (the [steady-state step E invariant-line token added in #332](./steady-state.md#e-invariant-line-end-of-every-steady-state-turn)) with the count returned by the wide fetch BEFORE the client-side filter runs — this is the surface the user (and the next session's orchestrator) reads to spot a "raw_backlog=0 but unfiltered_open_count=29" smell that would otherwise hide a regression in the client-side filter itself. Also stamp `me_assigned_open` (the [steady-state step E invariant-line token added in #1194](./steady-state.md#e-invariant-line-end-of-every-steady-state-turn)) with the count, from that same wide-fetch payload, of issues assigned to the gh-authenticated user — the narrower signal that names exactly the bucket a wrong assignee filter erases. Subtract the union of `in_flight` (target numbers), `ready_issues` (numbers), `raw_backlog` (numbers), `deferred_issues` (issue numbers), and any issues closed earlier this session (via PR auto-close) from the post-filter list. If the resulting net-new set is **non-empty**, append the new numbers to `raw_backlog` in priority order (same ranking as [setup.md step 4](./setup/04-backlog-divert.md#4-fetch--rank-the-backlog)), run scope-refill via step D, and retry dispatch from [steady-state step C](./steady-state.md#c-dispatch-a-replacement-if-work-remains--mandatory-action). **Do NOT proceed to drain.**

If the net-new set is empty, every row of the queue registry above AND step 4 have passed on internal-queue arithmetic alone — that arithmetic is necessary but **not sufficient** ([#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 3: "queues empty" and "the filter erased the work" are indistinguishable from queue counts alone). Build the completion ledger from this same fresh fetch — read [`setup/04f-completion-ledger.md`](./setup/04f-completion-ledger.md) now and run it in full: every open issue in the fetch must land in exactly one of its eleven buckets. **If any issue is unaccounted (bucket 11), that is a filter defect — log it loudly (issue numbers + why no bucket matched) and do NOT proceed to drain**; treat it the same as a non-empty net-new set above (append the unaccounted numbers to `raw_backlog`, retry dispatch). Only when the ledger places every open issue in a bucket AND bucket 10 (dispatchable) is empty may the orchestrator proceed to the [end-of-session drain](#end-of-session-drain) below — render the honest terminal shape (`0 dispatchable of <total> open — complete, see ledger`) rather than a bare "backlog empty."
