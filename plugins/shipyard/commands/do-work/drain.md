...
**Drain-mode termination**: when `draining = true` (see [Soft drain](#soft-drain)), the exit condition collapses to registry row 1 only (`in_flight` empty) — registry rows 2–7 and step 4 are all skipped, and every queue they'd otherwise check (`failed_prs` / `ready_issues` / `raw_backlog` / `investigate_candidates` / `divert_queue` / `operator_queue`) is left as-is and rebuilt (or re-drained) next session. The fresh-fetch in step 4 is deliberately bypassed: drain mode is the user's explicit "stop dispatching" signal, and surfacing new candidates would contradict that. The drain → cleanup → summary flow below still runs.
