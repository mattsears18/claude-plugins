...
> The session is complete only when **every open workable issue is closed or dispositioned** — merged via its resolving PR, or labeled to a genuine human/operator review class (`needs-human-review` / `agent-console`) — **AND every PR this session opened is merged**, or blocked on a confirmed **external dependency** the agent cannot itself perform (a secret value only a human can supply, an upstream release, a provider-console action). Those genuine external dependencies are the **only** legitimate residual stopping points. A PR that is merely red, DIRTY, or slow is not "complete" — it is unfinished tail the drain must keep driving toward merge.

The termination assertion below (the queue registry plus step 4) is the **dispatch-pool-empty gate** — the precondition for the orchestrator to stop dispatching new workers and enter the [end-of-session drain](#end-of-session-drain). It is necessary but **not sufficient** for session completion: the drain is where the completion condition above is pursued (drive the tail) and ultimately asserted, or where a **bounded-exit concession** is recorded when a PR can't be driven to merge within the session's safety bounds (see [End-of-session drain](#end-of-session-drain) → Completion vs bounded-exit). Do not treat passing the termination assertion as "session done."

**Any end-of-session follow-up/friction-issue filing runs BEFORE this assertion, never after — and never during or after [cleanup](./cleanup-summary.md#end-of-session-cleanup).** Some operator conventions (e.g. a maintainer's global instructions) direct the orchestrator to file GitHub issues for friction the session hit — spec gaps, misclassifications, ambiguous contracts — before considering the session done. File any such issues **before** running the termination assertion below, in the same turn the friction was identified — the assertion's own step 4 fresh-fetch (below) will then naturally pick up what was just filed as a net-new candidate and loop back through dispatch rather than terminating around it. File them per [`shipyard:filing-github-issues`](../../skills/filing-github-issues/SKILL.md) conventions, the same as any other shipyard-created issue — never an ad hoc `gh issue create` with no provenance label or milestone resolution: the `shipyard` session-stamp label (ensure-then-label), and — on a repo with `milestones.enabled` + `milestones.assign_on_file` both `true` — a milestone via that skill's § "Milestone assignment" (BET-matching against the open milestone list, falling back to `milestones.fallback` when nothing matches). An orchestrator-filed issue with no milestone resolution silently starves `milestones.prioritize_dispatch`'s ranking input exactly like an unmilestoned worker follow-up does (issue [#1421](https://github.com/mattsears18/shipyard/issues/1421)). If a friction issue is discovered mid-drain (after the assertion has already passed once), file it immediately and let [step 5's re-validation](#pre-drain-re-validation-of-deferred-entries) and the drain's own polling naturally re-admit it — do not defer the filing to "after this session wraps up." See [RATIONALE → Friction-issue filing must precede the termination assertion](../do-work-RATIONALE.md#friction-issue-filing-must-precede-the-termination-assertion-743) for why the ordering matters (the #743 repro) and the `bump-tokens --allow-degraded-init` re-entrancy safety net.

Before handing off to the drain (which then drives the tail → cleanup → summary → exit), the orchestrator MUST walk the **queue registry** below, then run **step 4** (the fresh-fetch verification) that follows it — plus the completion ledger described further below. **Enumerating a hand-picked subset of queues is itself a termination hole** (issue [#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 2: `raw_backlog` sat outside the original checks entirely, invisible to every gate, and `investigate_candidates` had the identical defect by a second route). The registry closes that hole **structurally**: a queue not represented as a row is a queue this assertion cannot see, so adding a new work-queue to session state means adding a row to the table below — **no other edit to this assertion is required** for the new queue to gate termination ([#1253](https://github.com/mattsears18/shipyard/issues/1253)). Each row (and step 4 itself) is a thing-to-do, not a thing-to-assert-against-stored-state — step 4 in particular must execute the live `gh` query, not infer the result from cached `raw_backlog` size.

### Queue registry

Walk the table **top to bottom, in the listed order**, checking each queue's current size **from working memory** (session state — never a live query; the live query is step 4 below). The **first** non-empty row takes its action and returns control **immediately**: do not check any row below it this turn, and do not proceed to step 4. The whole registry is walked again from the top on the next completion notification / dispatch turn, so a row's action only ever needs to make progress, not clear every row in one pass.

| # | Queue | Non-empty action | Why this queue is in the registry |
|---|---|---|---|
| 1 | `in_flight` | Do NOT proceed. Return control and let the existing in-flight workers finish; this row re-runs on the next completion notification. | The live dispatch pool, checked from the [orchestrator-state struct](../do-work.md#orchestrator-state)'s `in_flight` map — can't drain around live workers. |
| 2 | `failed_prs` | Fill the pool from `failed_prs` per [dispatch rule 2](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) and return control. | Red PRs from a prior turn still waiting on a fix-checks-only dispatch. |
| 3 | `ready_issues` | Fill the pool from `ready_issues` per [dispatch rule 3](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) and return control. | Scoped, dispatch-ready issues. |
| 4 | `raw_backlog` | Trigger [dispatch rule 5](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c)'s background scope-refill (moves entries into `ready_issues`) and return control; this row re-runs once the refill delivers. | The check that `raw_backlog` is empty is the fix for [#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 2: an issue can sit in `raw_backlog` having been fetched and ranked but never scoped into `ready_issues` — simultaneously "already known" (subtracted from step 4's net-new diff below) and "not ready" (invisible to row 3) — so nothing above this row would ever catch it. |
| 5 | `investigate_candidates` | Dispatch per [dispatch rule 1.5](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) and return control. | The check that `investigate_candidates` must be empty closes the identical defect `raw_backlog` had before row 4 existed — step 4's own client-side filter drops investigate-signal issues (they were already routed to `investigate_candidates` at setup, not left in the general fetch), so a routed-but-unworked candidate could never register as net-new either ([#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 2, second application). |
| 6 | `divert_queue` | Dispatch per [dispatch rule 1](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) and return control. | The check that `divert_queue` must be empty covers entries populated by [setup step 4.5](./setup/04-backlog-divert.md#45-divert-checks-main-ci--pr-pileup) and by step D's periodic refresh (main-CI red, PR pileup). |
| 7 | `awaiting_external` | Do NOT proceed. Run [step D's `awaiting_external` sweep](./steady-state.md#d-periodic-refresh) once — re-probe every entry, resume any whose job went terminal, expire any past `deadline_at` — and return control; this row re-runs on the next turn. | The [#1390](https://github.com/mattsears18/shipyard/issues/1390) park queue: a worker that returned `awaiting-external` did NOT finish its issue — it did everything it could and is waiting on a long external job. Terminating around a live entry would silently abandon near-complete work whose only remaining input was a clock, and would strand the parked agent's worktree. Bounded by `awaiting_external.max_hours` (default 2), so this row can never block termination indefinitely: on expiry the sweep converts the entry to a `blocked:` hand-back and the row clears. |
| 8 | `operator_queue` | Drain the next item on the main thread per [operate/01-queue-and-authorization.md](./operate/01-queue-and-authorization.md#draining) and return control. | Only meaningful when the operator layer is enabled (default; always empty under `--no-operate` / `--hands-off`, so this row is a permanent no-op there). **Exception:** on the degraded path (no browser backend reachable) the queue cannot be drained — its items are surfaced as hand-backs in the end-of-session summary instead, and do NOT block this row from reading empty. |
| 9 | `paused_on_environment` | Do NOT proceed. A pause is active — the resume watch (`scripts/watch-resume-probe.sh`, armed via `Monitor`) is what re-triggers this row's re-check, not a poll of this row itself; return control and let it fire. This row re-runs on the resume notification (clears the entry) or on expiry (degrades to a hand-back and clears the entry) — either way non-null becomes null and the row reads empty on the next pass. | The [#1402](https://github.com/mattsears18/shipyard/issues/1402) session-wide pause: `ci_backpressure` held with no worker left in flight to trigger a future retry (see [steady-state.md's "Session-wide environmental pause"](./environmental-pause.md)). Terminating around a live pause would abandon a still-workable backlog that is merely waiting on a transient, self-clearing environmental condition — the exact silent-stop the issue reports. Bounded by `paused_on_environment.max_hours` (default 4, anchored at first pause), so this row can never block termination indefinitely. |

Once every row above reports empty (subject to row 8's degraded-path exception), proceed to step 4.

4. **Fresh-fetch verification.** Run the canonical step-4 backlog query **now** — against the live tracker, not stored state. **Use the same wide-fetch shape that [setup.md step 4](./setup/04-backlog-divert.md#4-fetch--rank-the-backlog) uses** (the previous narrower `-linked:pr` + `-label:...` server-side qualifiers were removed in [#332](https://github.com/mattsears18/shipyard/issues/332) — they silently excluded resumable-work issues like prior-session self-assigns whose linked PR had been closed or never opened, producing premature "backlog empty → drain" handoffs):

   ```bash
   # Wide fetch — server-side filter is purely --state open. All
   # eligibility checks happen in scripts/backlog-filter.sh classify (see
   # setup.md step 4's "Invocation" block for the canonical call shape).
   FRESH_FETCH_JSON=$(gh issue list --repo <owner/repo> --state open --limit 200 \
   ...
   **Stamp the invariant-line tokens immediately, from this same payload, BEFORE classification runs** ([#1246](https://github.com/mattsears18/shipyard/issues/1246)) — `scripts/backlog-filter.sh summary` is the single-source-of-truth implementation (the same pure-function split as `classify` itself — see that script's header comment), so this count can never drift from what [step E's invariant line](./steady-state.md#e-invariant-line-end-of-every-steady-state-turn) reports:
   ...
   # Herestrings again, for the same #1289 reason as the SUMMARY line above.
   ...
   `unfiltered_open_count` (the [steady-state step E invariant-line token added in #332](./steady-state.md#e-invariant-line-end-of-every-steady-state-turn)) is the count returned by the wide fetch BEFORE classification runs — this is the surface the user (and the next session's orchestrator) reads to spot a "raw_backlog=0 but unfiltered_open_count=29" smell that would otherwise hide a regression in the classifier itself. `me_assigned_open` (the [steady-state step E invariant-line token added in #1194](./steady-state.md#e-invariant-line-end-of-every-steady-state-turn)) narrows that same wide-fetch payload to the count of issues assigned to the gh-authenticated user — the narrower signal that names exactly the bucket a wrong assignee filter erases **when `backlog.respect_assignees == true`; with the default `false` (issue [#1248](https://github.com/mattsears18/shipyard/issues/1248)) the assignee-based drop clause never runs, so this reading no longer applies — see [steady-state.md's purpose-narrowing note](./steady-state.md#e-invariant-line-end-of-every-steady-state-turn) for the full explanation.** This same `unfiltered_open_count` value feeds [`setup/04f-completion-ledger.md`](./setup/04f-completion-ledger.md)'s `<total>` below — the ledger's own per-bucket accounting is what makes a divergence loud rather than merely recorded: an issue the ledger can't place lands in bucket 11 (Unaccounted) and blocks termination outright.

   **Classify by invoking `scripts/backlog-filter.sh classify` — never re-derive the predicate in this file, and never substitute a hand-rolled shorthand for it** (that's exactly the bug [#332](https://github.com/mattsears18/shipyard/issues/332) documented, one layer down, and it recurred client-side in a mid-drain ad-hoc query as recently as [#1194](https://github.com/mattsears18/shipyard/issues/1194); a *third* divergence sat committed in THIS file's own prose until [#1247](https://github.com/mattsears18/shipyard/issues/1247) converged all three call-sites onto the script — `needs-triage` (a label since retired in [#1120](https://github.com/mattsears18/shipyard/issues/1120)) used to be listed inside this step's dispatch-gate drop enumeration, silently dropping every trusted-author untriaged/Sentry-authored issue at session end instead of routing it to `investigate_candidates` the way setup.md step 4 always did, so an investigate candidate discovered only late in a session could never block termination by any path). Pass the same flags [setup.md step 4](./setup/04-backlog-divert.md#4-fetch--rank-the-backlog)'s "Invocation" block documents (`--me`, `--trusted-authors`, `--closed-by-healthy-pr`, `--closed-by-open-pr`, `--peer-claimed`, `--investigate-dispatch`, `--respect-assignees`, `--milestones-enabled`, `--milestones-prioritize-dispatch`, `--recheck-probe-enabled`, `--probe-verdicts` — the latter two per issue [#1356](https://github.com/mattsears18/shipyard/issues/1356), re-run `eval-probes` against `$FRESH_FETCH_JSON` the same way, gated on `scope.recheck_probe_enabled`; `--closed-by-open-pr` per issue [#1389](https://github.com/mattsears18/shipyard/issues/1389), re-run `closed-by-open-pr` here too — at drain time nearly every session PR is open and many are mid-CI, so a stale or omitted map is the single most likely way for this assertion to count an already-covered issue as net-new dispatchable work) against this same `$FRESH_FETCH_JSON` and project the script's `eligible` numbers to a sorted list for the diff below (the milestone flags don't change *membership* in that list, only rank order among the net-new appends two paragraphs down — pass them anyway so `raw_backlog`'s tail order stays consistent with the other two call-sites); separately project its `route:investigate` numbers for the `investigate_candidates` diff two paragraphs down. `blocked:agent-hard`/legacy `blocked:agent`, `needs-design`, the `needs-decomposition`/`tracking` pair, and `needs-refinement` were eliminated per the binary-backlog migration and no longer appear in the script's `gate_labels` set — see [RATIONALE → Label-lifecycle provenance](../do-work-RATIONALE.md#label-lifecycle-provenance--folded--eliminated-gate-labels) for the full fold/elimination history. `agent-console` is a *route*, not a gate label — the script routes it exactly as [`04g-operator-routing.md`](./setup/04g-operator-routing.md) documents (drained by the operator phase by default, surfaced as a hand-back under `--no-operate` / `--hands-off`), never a drop.

   Subtract the union of `in_flight` (target numbers), `ready_issues` (numbers), `raw_backlog` (numbers), `deferred_issues` (issue numbers), and any issues closed earlier this session (via PR auto-close) from the `eligible` list. If the resulting net-new set is **non-empty**, append the new numbers to `raw_backlog` in the script's own rank order (no separate sort pass needed), run scope-refill via step D, and retry dispatch from [steady-state step C](./steady-state.md#c-dispatch-a-replacement-if-work-remains--mandatory-action). **Separately, subtract `investigate_candidates` (numbers) and `in_flight` from the script's `route:investigate` list; a non-empty result here is equally disqualifying** — append the net-new numbers to `investigate_candidates` in the script's investigate rank order and retry dispatch the same way; a trusted-author untriaged/Sentry-authored issue discovered only at drain time must reach `investigate_candidates`, never be silently dropped. **Do NOT proceed to drain** while either net-new set is non-empty.

If both net-new sets are empty, every row of the queue registry above AND step 4 have passed on internal-queue arithmetic alone — that arithmetic is necessary but **not sufficient** ([#1250](https://github.com/mattsears18/shipyard/issues/1250) Hole 3: "queues empty" and "the filter erased the work" are indistinguishable from queue counts alone). Build the completion ledger from this same fresh fetch — read [`setup/04f-completion-ledger.md`](./setup/04f-completion-ledger.md) now and run it in full: every open issue in the fetch must land in exactly one of its eleven buckets. **If any issue is unaccounted (bucket 11), that is a filter defect — log it loudly (issue numbers + why no bucket matched) and do NOT proceed to drain**; treat it the same as a non-empty net-new set above (append the unaccounted numbers to `raw_backlog`, retry dispatch). Only when the ledger places every open issue in a bucket AND bucket 10 (dispatchable) is empty may the orchestrator proceed to the [end-of-session drain](#end-of-session-drain) below — render the honest terminal shape (`0 dispatchable of 0 open — empty, see ledger` or `0 dispatchable of <total> open — fully-gated, see ledger (<breakdown>)`, per the ledger's own three-shape rendering — never a bare "backlog empty" or unqualified "complete"; the stopped-short sibling shape is `<N> dispatchable of <total> open — stopped on <bound>, see ledger`).

**A drained dispatch pool with open issues remaining is a distinct terminal state — `fully-gated` — never conflated with `empty` ([#1358](https://github.com/mattsears18/shipyard/issues/1358)).** The dispatch-pool-empty gate above (queue registry + step 4's net-new check) drains identically whether `<total>` open-issue count is 0 (nothing is open — `empty`) or > 0 with every issue parked in one of buckets 1–9 (nothing is *dispatchable*, but plenty is still open and waiting on a clock, a decision, or a console action — `fully-gated`). Both are legitimate, non-defective ways to reach this point; neither should print as an unqualified "complete." The ledger's terminal-message rendering (`setup/04f-completion-ledger.md`'s "Render the terminal message" section) is the single source of truth for which of the three shapes (`empty`, `fully-gated`, or `stopped on <bound>`) applies — use it verbatim rather than re-deriving the distinction here, and carry the same `empty`/`fully-gated` naming through to the [end-of-session summary](./cleanup-summary.md#end-of-session-summary)'s `Session end` line so a human reading the summary alone (not just this internal gate) can tell the two apart at a glance.

   **The workable count is MECHANICAL, never discretionary ([#531](https://github.com/mattsears18/shipyard/issues/531)).** The post-filter list is produced by [`scripts/backlog-filter.sh classify`](./setup/04-backlog-divert.md#4-fetch--rank-the-backlog) alone — trusted-author check, dispatch-gate labels, assignee≠@me (only when `backlog.respect_assignees == true`; skipped by default per [#1248](https://github.com/mattsears18/shipyard/issues/1248)), `Blocked by #N` still-open, `do-work-blocked-until` still in the future (TIME-gated), an unresolved `do-work-recheck` probe verdict on an issue carrying that companion marker (EVENT-gated — issue [#1356](https://github.com/mattsears18/shipyard/issues/1356), regardless of whether the paired calendar date has elapsed), closed-by-@me-authored-healthy-PR — and **nothing else**. The orchestrator MAY NOT judgment-exclude a candidate from this list. "It's a follow-up I filed this session" / "it's a meta-issue about the orchestrator" / "it rewrites the reconcile path I'm running" / "feels like it should wait" are NOT filter criteria — none appears in the script's classification, and none matches a [`defer_reason_class`](./setup/06-scope-preflight.md#6-initial-scope-pre-flight) (`external-dependency`, `human-decision-required`, `untrusted-author`, `confirmed-blocker-still-open`, `confirmed-non-shippable-as-single-PR`, `time-gated`, `blocked-by-in-flight-pr`). An issue that passes the mechanical filter is workable; if the mechanical net-new count is **> 0**, the loop MUST NOT terminate — it MUST append the candidates to `raw_backlog` and dispatch. The only sanctioned ways off the dispatch queue are the seven `defer_reason_class` defers (each re-validated by [step 5](#pre-drain-re-validation-of-deferred-entries) before drain fires) and the mechanical exclusions enumerated above; constructing an "empty net-new set" by quietly dropping mechanically-workable issues is the exact bypass [#531](https://github.com/mattsears18/shipyard/issues/531) documents (a session drained on workable self-filed follow-ups #529 / #530 because the orchestrator judgment-excluded its own just-filed issues from "workable," then declared termination — forcing the maintainer to manually re-instruct it to finish them). See also the dont.md rule ["Don't hand a workable issue to the human"](./dont.md).
...
**Drain-mode termination**: when `draining = true` (see [Soft drain](#soft-drain)), the exit condition collapses to registry row 1 only (`in_flight` empty) — registry rows 2–8 and step 4 are all skipped, and every queue they'd otherwise check (`failed_prs` / `ready_issues` / `raw_backlog` / `investigate_candidates` / `divert_queue` / `awaiting_external` / `operator_queue`) is left as-is and rebuilt (or re-drained) next session. A live `awaiting_external` entry at this point is surfaced as a hand-back in the end-of-session summary — a parked worker whose external job outlived the session is exactly the kind of unfinished tail the summary exists to name. The fresh-fetch in step 4 is deliberately bypassed: drain mode is the user's explicit "stop dispatching" signal, and surfacing new candidates would contradict that. The drain → cleanup → summary flow below still runs.

### Pre-drain re-validation of deferred entries

Once the termination assertion passes (all queues empty, live backlog returns no net-new), the orchestrator MUST run a fifth step **before transitioning `drain.active = true`** — whenever `deferred_issues` contains any entries at all (regardless of provenance):
...
1. **Re-read the defer reason.** Does it name a specific blocker issue or PR (e.g., "Gated on #1077", "Blocked on #1052")?
   ...
   - If **no** (the reason is a free-form judgment, not a named blocker): **dispatch a scope agent** for this issue now — but **first re-run the [pre-scope orchestrator-side detector batch](./setup/06-scope-preflight.md#pre-scope-orchestrator-side-detectors-synthetic-defers) from setup.md step 6** against the issue body. If any detector fires, synthesize the deferred entry directly (skip the scope-agent dispatch) and leave the entry in `deferred_issues` with the fresh synthesized fields — same behavior as the initial pre-flight, applied to re-validation. This keeps the detector load-bearing across both call sites; without re-running it here, a re-validation that dispatches a scope agent against a body the detector batch guards (a `.github/workflows/`-proposing body, a `.claude/settings.json` / `.claude/settings.local.json` / `.mcp.json` / `.claude/hooks/`-proposing body, or any future detector's target) would likely return `ready` (the scope agent has no detector built in) and immediately reproduce the orphan-branch / classifier-denial failure mode the detector exists to prevent. (Detector 2's matched path set is config-driven via `scope.self_modification_paths` and includes `.claude/hooks/` by default — issue [#591](https://github.com/mattsears18/shipyard/issues/591); the re-run resolves the same effective array.) If no detector fires, dispatch the scope agent normally:
     ...
     - If the scope agent returns the **already-landed shape** ([#992](https://github.com/mattsears18/shipyard/issues/992)): apply the same close-and-comment handling as [setup.md step 6's Already-landed entries](./setup/06c-scope-handling-ui.md#handling-each-returned-entry-fires-as-each-background-agent-completes) — post the comment, close with `--reason completed`, then remove the entry from `deferred_issues` (do NOT push to `raw_backlog` — there's nothing left to schedule). Log: `[pre-drain-revalidate] #<N> was orchestrator-judgment deferred but scope agent found it already-landed on origin/<default-branch>; closed`.
...
2. **`entry.evidence_pointer` re-validates mechanically right now.** Run the same per-class shape check from [setup.md step 6's per-class validator](./setup/06b-scope-carveouts.md#per-class-evidence-shapes--what-evidence_pointer-must-look-like) against the existing `evidence_pointer`, then run the cheap class-specific ground-truth probe:
   ...
   - `blocked-by-in-flight-pr` ([#1426](https://github.com/mattsears18/shipyard/issues/1426)) → shape check (pointer starts with `Blocked by in-flight PR:`, `blocking_prs` present and non-empty), plus re-run `gh pr view <N> --repo <owner/repo> --json state -q .state` for every PR in `blocking_prs`; all must still be `OPEN`. If any has merged or closed, condition 2 fails — the collision has resolved and the issue may now be genuinely ready (mirroring how a resolved `confirmed-blocker-still-open` blocker fails this same condition above), which routes to the fresh scope-agent dispatch below rather than skipping straight to `ready_issues` — the point-2 "reuse the cached scope without paying for a second dispatch" optimization is deliberately not wired in this revision (see [#1426](https://github.com/mattsears18/shipyard/issues/1426)'s tracking follow-up).
   ...
   - `time-gated` ([#1165](https://github.com/mattsears18/shipyard/issues/1165)) → shape check, plus re-run the ground-truth probe: today's UTC calendar date (`date -u +%F`) must still be strictly before the pointer's `YYYY-MM-DD` date. If the date has elapsed, condition 2 fails (the defer is stale — the issue may now be genuinely ready, mirroring how a resolved `confirmed-blocker-still-open` blocker fails this same condition above).
...
2. If no issues were re-admitted (all defers were confirmed still-valid by their respective re-validation): emit the **pre-drain audit banner** and proceed to drain:
   ...
   PRE-DRAIN AUDIT
       ...
       defer_reason_class breakdown:
           ...
           time-gated: <c_time_gated>
           blocked-by-in-flight-pr: <c_pr_collision>
       all defers re-validated — blockers still active, scope agents re-confirmed
       last live backlog fetch: <HH:MM:SS UTC>
       completion ledger (#1250): <total_open> open · 0 dispatchable · 0 unaccounted — terminal state: <empty|fully-gated> — see setup/04f-completion-ledger.md
       proceeding to drain
   ```

   The `completion ledger` line restates the [step 4 fresh-fetch's ledger result](#termination-assertion) — by the time this banner prints, the ledger has already gated `0 dispatchable · 0 unaccounted` (a non-zero value on either count would have looped back to dispatch instead of reaching 5.c at all). Printing it here makes the honest accounting visible in the audit trail a human actually reads, not just in the internal gate that produced it. `<total_open>` is the ledger's own open-issue count from that same fetch. **`terminal state`** is `empty` when `<total_open>` is 0, `fully-gated` when `<total_open>` is > 0 — the same two-way naming `setup/04f-completion-ledger.md`'s terminal-message rendering defines, carried into this audit banner so it's never lost between the internal gate and the eventual [end-of-session summary](./cleanup-summary.md#end-of-session-summary) ([#1358](https://github.com/mattsears18/shipyard/issues/1358)).
...
Once the termination assertion passes AND step 5 re-validation clears (or was a no-op), run end-of-session drain → cleanup → summary.
...
### Worktree-safe polling pattern ([#1326](https://github.com/mattsears18/shipyard/issues/1326))

Every poll loop this file describes — the drain protocol's own "every 60s, snapshot the open PRs" bookkeeping below, the [mutually-blocking PR detection](#mutually-blocking-pr-detection-drain-phase-surfacing-only--1140) scan, a main-CI recheck, a `ci.settled_minutes` head-SHA-unchanged wait — is written as prose because the orchestrator's actual polling mechanism cannot be a literal inline `while true; do ... done` (or `for`) Bash block. Post-relocation, [`dont.md`'s "Post-relocation Bash blocks must be plain, single-purpose commands"](./dont.md#post-relocation-bash-blocks-must-be-plain-single-purpose-commands-1277) rule already establishes that a loop wrapping `gh`/`git` calls is refused by the worktree-isolation guard as too complex to verify it stays inside the worktree — and **a poll loop is exactly that shape**: its entire purpose is repeating a `gh pr view`/`gh pr list` call an unbounded, data-dependent number of times, so it is refused on sight, with no partial-credit variant that slips through.

**The refusal message's own remedy — "break it into plain, separate commands" — does not work for a poll loop.** That advice fits `dont.md`'s "decompose" branch: a short, bounded, known-at-authoring-time sequence. A poll loop's iteration count is neither bounded nor known ahead of time; there is no way to "break it into" a fixed number of separate `Bash` tool calls without literally re-issuing a fresh tool call on every single poll tick for the entire drain, which defeats the point of polling in the first place.
...
1. **Prefer the shipped helper — [`scripts/watch-pr-terminal.sh`](../../scripts/watch-pr-terminal.sh) — over hand-authoring a new loop.** It implements exactly this pattern for the single-PR case: poll `gh pr view <N> --repo <owner/repo>` on a bounded interval (default 60s, matching this file's own per-poll cadence) up to a bounded max-wait ceiling (default 7200s = 2h), and emit exactly one stdout line when the PR reaches a terminal state — `merged`, `closed`, or a `failing` check — or a `timeout` line if it never does within the ceiling. Invoke it as one plain command:
   ...
   **Sibling — [`scripts/watch-resume-probe.sh`](../../scripts/watch-resume-probe.sh) (issue [#1402](https://github.com/mattsears18/shipyard/issues/1402)).** The identical extraction pattern applied to a genuinely *recurring, spec-defined* poll rather than an ad hoc one-off — [steady-state.md's "Session-wide environmental pause"](./environmental-pause.md) arms it via a background `Monitor` call to re-check a saturated self-hosted CI pool without a worker in flight to trigger a future retry any other way. Committed (rather than left to sub-step 2's per-session scratch-script pattern below) because this poll shape recurs across sessions by design, the same reasoning that made `watch-pr-terminal.sh` a committed script instead of a one-off scratch file.

2. **When the poll doesn't fit the single-PR shape** (this file's own multi-PR batched bookkeeping, or a condition the helper doesn't check), write the loop body to a scratch script instead of inlining it, then invoke that script as one plain command — the same `Write`-then-`bash <path>` shape `shipyard:worker-preamble`'s [`compound-command-refusal.md`](../../skills/worker-preamble/compound-command-refusal.md) fragment documents for a dispatched worker, applied here to the orchestrator's own drain loop.

**Where the scratch script lives — worktree-local, never outside the worktree.** A tempting first instinct is to stage a throwaway poll script somewhere outside the worktree — a session/job temp directory, `/tmp` — on the theory that it's disposable scratch and doesn't belong in the repo. **Don't.** A worktree-isolated session's `Bash` guard refuses a shell redirect *target* outside the worktree on an otherwise-plain, non-git command too — see [issue #1325](https://github.com/mattsears18/shipyard/issues/1325) and [`bash-refusal-triggers.md`](./bash-refusal-triggers.md) for the general refused-shape catalog this class of trigger belongs to — and authoring a scratch *script file* at `$CLAUDE_JOB_DIR/tmp/...` or a bare `/tmp/...` path is the identical shape: a path outside this worktree. Use the existing untracked `.shipyard-*` scratch-file convention instead — the same flat, worktree-root convention [`setup/00-config-worktree.md` step 0.5](./setup/00-config-worktree.md#05-move-into-the-orchestrators-worktree) already uses for `.shipyard-plugin-root` / `.shipyard-fetched-issues.json`: `Write` the loop body to `.shipyard-scratch-<name>.sh` at the orchestrator worktree's root (never `git add`ed, so it never lands in a commit; reaped with the rest of the worktree at end-of-session), then invoke it with `bash .shipyard-scratch-<name>.sh <args>` as one plain command. The `Write` tool itself is not a shell command and isn't subject to the isolation guard at all — only a subsequent shell *redirect* to an out-of-worktree path would trip it, and this convention never issues one.
...
Also initialize six per-session structures that gate fix-rebase re-dispatch, progress-based termination, and mutually-blocking-pair diagnosis:
...
- `mutually_blocking_pairs_detected = {}` ([#1140](https://github.com/mattsears18/shipyard/issues/1140)) — the per-session set of unordered PR-number pairs (keyed `"<min>,<max>"`) that the [mutually-blocking PR detector](#mutually-blocking-pr-detection-drain-phase-surfacing-only--1140) has already diagnosed this session. Purely a dedup counter for the [end-of-session summary](./cleanup-summary.md#end-of-session-summary) — membership does NOT change dispatch routing or the drain's settled definition (the per-PR idempotency check on the diagnosis comment itself, not this counter, is what prevents re-posting on every poll; this counter exists only so the summary line reports a per-session count rather than a per-poll one).
...
- `dirty_fix_checks_prs = {}` ([#1060](https://github.com/mattsears18/shipyard/issues/1060)) — the per-session set of PR numbers that have returned `dirty #<M>` (explicit or narrative-synthesized, or downgraded from an over-claimed `green`) from **any** fix-checks-only dispatch this session, per [steady-state.md step A's fix-checks reconcile](./steady-state.md#a-reconcile-the-return). This can still happen even after the routing fix above — a PR dispatched to fix-checks-only mid-session (via step C, while it was still red-but-not-yet-DIRTY) can discover it went DIRTY between dispatch and the worker starting, from an unrelated sibling merge. Read alongside `rebase_blocked_prs` to detect the deadlock signature below.
- `deadlocked_prs = {}` ([#1060](https://github.com/mattsears18/shipyard/issues/1060)) — the per-session set of PR numbers for which **both** `dirty_fix_checks_prs` membership AND `rebase_blocked_prs` membership hold — i.e. a PR that returned `dirty` from a fix-checks dispatch AND `blocked rebase` from a fix-rebase dispatch within the same session. Written by [steady-state.md step A's blocked-rebase reconcile](./steady-state.md#a-reconcile-the-return) the moment a `blocked rebase #<M>` return is reconciled for a PR already in `dirty_fix_checks_prs`. `deadlocked_prs` is a **subset** of `rebase_blocked_prs`, not a disjoint structure — membership in it doesn't change how the PR is gated (it's still settled, still excluded from re-dispatch, exactly like any other `rebase_blocked_prs` member); it exists purely so [cleanup-summary.md's end-of-session summary](./cleanup-summary.md#end-of-session-summary) can surface this specific history distinctly rather than folding it into an undifferentiated `blocked` entry — a PR that hit this combo had a documented cycling history even though, post-#1060, the `blocked rebase` it ultimately returned reflects a genuine non-trivial conflict rather than the old #577 routing artifact.
...
**Run the primary-checkout branch-leak guard once at drain entry** ([#387](https://github.com/mattsears18/shipyard/issues/387)). Drain is exactly where a leaked `do-work/*` checkout on the primary does its damage: it holds git's per-branch lock on a PR head branch, so the [pre-dispatch head-branch reap](#pre-dispatch-head-branch-reap-self-pid-lock-release) below and the fix-rebase worker's `git switch <head>` both collide with it. The [#387](https://github.com/mattsears18/shipyard/issues/387) repro surfaced precisely here — drain dispatching a fix-rebase for the one DIRTY PR (#384, head `do-work/issue-378`) against a primary the harness had leaked onto `do-work/issue-378`. Restoring the primary to the default branch at drain entry frees the head branch *before* the first per-PR reap runs. Run the **exact** guard from [steady-state.md step A.0.6](./steady-state.md#a06-primary-checkout-branch-leak-guard-fires-every-reconcile-turn-before-a1) — the same `primary-leak-guard.sh run --repo <owner/repo>` invocation, the same `verdict=` parsing and logging per outcome, the same `primary_leak_counters` increments — it is the single source of truth; do not re-derive a variant here, and do NOT reach for an inline `git -C "$PRIMARY_CHECKOUT" ...` even though drain, like steady-state, runs from a worktree-isolated orchestrator session where such a call would be refused (issues [#1316](https://github.com/mattsears18/shipyard/issues/1316) / [#1323](https://github.com/mattsears18/shipyard/issues/1323)).

The two structures are intentionally distinct: `rebase_blocked_prs` is the **deterministic-failure** gate (the same conflict will produce the same outcome on retry — don't waste a worker), `rebase_success_counts` is the **rate-limit** gate (the merge train is racing rebases — cap the spend without giving up on the first race). See [RATIONALE → Rebase gates: deterministic-failure vs rate-limit](../do-work-RATIONALE.md#rebase-gates-deterministic-failure-vs-rate-limit-265) for the race this split fixes.
...
`mergeStateStatus`, `statusCheckRollup`, and `headRefName` / `headRefOid` are required for the `R_new` / `D_dirty` / `D_dirty_red` split below. `mergeStateStatus == "DIRTY"` routes a PR to `fix-rebase` (`D_dirty`) **regardless of check state** — a DIRTY PR that's also red (`D_dirty_red`) is a distinguished, informational subset of `D_dirty`, tracked separately purely for status-line visibility, not a different dispatch target ([#1060](https://github.com/mattsears18/shipyard/issues/1060), superseding [#577](https://github.com/mattsears18/shipyard/issues/577) — see [RATIONALE](../do-work-RATIONALE.md#1060-supersedes-577--dirty-and-red-routes-to-fix-rebase-not-fix-checks)). Only a **non-DIRTY** red PR (`R_new`) routes to `fix-checks`. The head-branch identifiers are passed into the fix-rebase prompt template. **Never classify a DIRTY PR as fix-checks-eligible** — a DIRTY PR has no merge ref, so fix-checks-only's own DIRTY short-circuit would bail immediately without attempting anything.
...
- `R_new` = PRs whose rollup contains a hard failure (`FAILURE` / `ERROR` / `TIMED_OUT` / `CANCELLED` / `ACTION_REQUIRED`) **on the latest run per check name** AND `mergeStateStatus != "DIRTY"` AND are NOT carrying `blocked:ci` AND are not already in `in_flight` or `failed_prs`. **The `mergeStateStatus != "DIRTY"` exclusion is load-bearing as of [#1060](https://github.com/mattsears18/shipyard/issues/1060)** — a DIRTY-and-red PR must route to `fix-rebase` (`D_dirty`/`D_dirty_red` below), never to `fix-checks`; without this exclusion a DIRTY-and-red PR would be double-counted into `R_new` and dispatched to fix-checks-only, which would immediately trip its own DIRTY short-circuit and bail with `dirty #<M>` — the exact wasted-dispatch cycle #1060 closes.
- `D_dirty` = PRs whose `mergeStateStatus == "DIRTY"` AND are NOT in `in_flight` AND are NOT in `rebase_blocked_prs` (the prior dispatch returned `blocked rebase` — re-dispatching would produce the same conflict) AND have `rebase_success_counts[<pr>] < 3` (PRs that have been successfully rebased 3 times in this session are rate-limit-capped and treated as settled — the merge train is advancing faster than rebase can keep up, defer to next session). **Check state no longer excludes a PR from `D_dirty`** ([#1060](https://github.com/mattsears18/shipyard/issues/1060), superseding [#577](https://github.com/mattsears18/shipyard/issues/577) — see [RATIONALE](../do-work-RATIONALE.md#1060-supersedes-577--dirty-and-red-routes-to-fix-rebase-not-fix-checks)) — a DIRTY PR routes to `fix-rebase` regardless of whether it's also carrying a failing check, because no check can be refreshed while DIRTY (a red check is a fossil of the last buildable base, not live evidence). A PR that was successfully rebased earlier this session and has since gone DIRTY again from a sibling merge IS in `D_dirty` as long as its success-count is under cap — a successful rebase doesn't gate future re-dispatches the way a `blocked rebase` does. **Always compute `D_dirty` using `mergeStateStatus` alone** — the rollup is no longer part of `D_dirty`'s own eligibility test (it still feeds `D_dirty_red`, purely for the status-line breakdown below).
- `D_dirty_red` = the **informational subset** of `D_dirty` (same membership test above) that ALSO has **at least one** hard-failure check **on the latest run per check name**. This is NOT a separate dispatch target — every PR in `D_dirty_red` is also in `D_dirty` and is dispatched identically to `fix-rebase` in per-poll action 2. `D_dirty_red` exists purely so the drain log and the `dirty_red=` status-line token (below) can show the operator which DIRTY PRs are also carrying a stale, frozen check failure that the rebase itself will supersede once a merge ref exists again. **Never dispatch a fix-checks-only worker against a PR in `D_dirty_red`** — fix-checks-only's own DIRTY short-circuit returns `dirty #<M>` immediately without attempting anything, wasting the dispatch (this is the inverse of the pre-#1060 rule, which routed `D_dirty_red` to fix-checks and left it unable to make progress once #1015 added that short-circuit).
...
**Latest-per-name semantics for `R_new` and `D_dirty_red`** (`D_dirty` itself no longer reads the rollup — see above). `statusCheckRollup` returns every check run for the PR's head SHA — including superseded runs (a check that ran, failed, was re-triggered, and passed appears twice). De-duplicate by check name and take the most recent entry per check (by `completedAt`, fallback `startedAt`) before computing either set below. See [RATIONALE → Latest-per-name rollup de-duplication](../do-work-RATIONALE.md#latest-per-name-rollup-de-duplication-333) for the naïve-walk failure modes this prevents and the session repro that motivated it.

```bash
# Latest-per-name failure count. Used by R_new (>0 AND NOT DIRTY → fix-checks) and
...
A PR is **settled** when any of: it's merged/closed, it's labeled `blocked:ci`, it has had a `blocked rebase` return this session (membership in `rebase_blocked_prs` — which includes `deadlocked_prs`, since that's a subset, not a disjoint gate; see below), it has hit the rate-limit cap (`rebase_success_counts[<pr>] >= 3` — three successful rebases in one session, but the merge train keeps re-DIRTY-ing it; surrender for this session and let the next pick it up), or membership in `P_settled` (its checks are all pending AND its head commit has been unchanged for at least `settled_minutes`). The progress signal — per-PR head-commit movement, not the wall clock — is what the `settled_minutes` threshold reads (issue [#374](https://github.com/mattsears18/shipyard/issues/374)). A PR actively churning (head moving every few minutes from rebases or pushes) never reaches the `settled_minutes` threshold and keeps the drain alive; a PR whose head went quiet 20+ min ago is settled regardless of how long the overall session has been running.

**`deadlocked_prs` membership doesn't change the settle test above — it changes how the PR is *reported* ([#1060](https://github.com/mattsears18/shipyard/issues/1060)).** A `deadlocked_prs` member is already settled by ordinary `rebase_blocked_prs` membership; the separate set exists so [cleanup-summary.md](./cleanup-summary.md#end-of-session-summary) can name it distinctly in the unfinished tail — "this PR returned `dirty` from fix-checks AND `blocked rebase` from fix-rebase this session" is a documented cycling history worth a human's attention on its own, even though the settle mechanics (and the drain's obligation to keep polling everything else) are unaffected. Before #1060, the analogous PR would have kept bouncing between fix-checks and fix-rebase indefinitely (the deadlock the issue reports) while counting as settled the whole time — silently misreporting a stuck PR as fine. Post-#1060 that specific infinite bounce can't recur (a DIRTY PR never reaches fix-checks at all), so a PR landing in `deadlocked_prs` today reflects a genuine non-trivial conflict discovered after an earlier, unrelated `dirty` observation — worth flagging by name, but not the unbounded cycle the original report described.
...
**The failure mode.** A fresh drain-phase worker lands in its own isolated worktree, then runs the safe two-step (`git fetch origin <head>` + `git switch <head>`) to land on the PR's head branch (`fix-checks-only.md` step 1 / `fix-rebase.md` step 1). When the original issue-work worker's worktree is *still locked* against that head branch — because the worker returned `blocked` / `errored` (so the A.1 `shipped`-only reap never ran for it), or a transient `gh` failure aborted the A.1 reap — `git switch` fails with *"is already checked out at \<path\>"* and the worker bails with `blocked rebase #<M>: head branch <HEAD_REF> locked in another worktree` (`fix-rebase.md` step 1) or `Cannot apply fix to PR branch — <head> is locked by another worktree` (fix-checks-only). The fresh worker has already identified the correct fix; it just can't push it. Since the orchestrator's own end-of-session reap (cleanup-summary step 3b) runs *after* drain, drain-phase dispatches always race the lock — and drain is exactly where fix-rebase / fix-checks dispatches are supposed to land ([`dont.md`](./dont.md) bans fix-rebase outside drain). See [RATIONALE → Pre-dispatch head-branch reap](../do-work-RATIONALE.md#pre-dispatch-head-branch-reap-session-reproductions-370--576) for the two session reproductions that motivated this.

**Why it's safe to reap.** The lock holds *our orchestrator's* PID (the harness writes the orchestrator PID into every dispatched agent's worktree lock). `classify-lock` short-circuits such a lock to `self-ancestor` once `SHIPYARD_ORCHESTRATOR_PID` is declared — which means "this lock is held by an orchestrator that is itself / an ancestor of ours, about to retire its own worktree." The original worker's return was already reconciled at [step A](./steady-state.md#a-reconcile-the-return) by the time drain runs; its worktree is logically done. This is the same self-ancestor reap logic [setup.md step 3b](./setup/01c-label-recovery-refine.md#3-ensure-label-exists--recover-from-prior-session) runs at session start and the A.1 `shipped`-immediate reap (#282) runs on issue-work completion — extended to the mid-session drain dispatch site for PRs whose originating worker did NOT take the `shipped` path (so A.1 never reaped them).

**Force-reap even on `peer-alive` (and `unknown`) at the drain dispatch site.** When the lock's classification is `peer-alive` (a live PID that `SHIPYARD_ORCHESTRATOR_PID` did not short-circuit to `self-ancestor`) — or `unknown` (issue #1206: the lock file exists but couldn't be parsed at all; treated identically) — the general-purpose drain pre-dispatch reap defers conservatively — **except** for worktrees whose HEAD ref matches the `do-work/issue-*` pattern: at drain time all issue-work workers in this session have already returned their terminal strings, so a `do-work/issue-<N>` worktree still `peer-alive`/`unknown` is completed-and-logically-done, not a live worker. Override to force-reap those (distinguishing them from live fix-checks or fix-rebase workers whose lock must NOT be yanked), and audit the override with classification `peer-alive-force-drain` / `unknown-force-drain` so it's distinguishable from the A.1 `peer-alive-force` in `~/.shipyard/reap-audit.jsonl`. See [RATIONALE → Pre-dispatch head-branch reap](../do-work-RATIONALE.md#pre-dispatch-head-branch-reap-session-reproductions-370--576) for why this is safe and how it closes the #576 failure path.
...
### Mutually-blocking PR detection (drain phase, surfacing only — [#1140](https://github.com/mattsears18/shipyard/issues/1140))

Before the per-poll actions below dispatch anything, scan for a **mutually-blocking pair**: two open `session_prs` PRs where each PR's failing *required* check(s) are fully covered by the other PR's passing checks — neither can go green alone, and nothing today records why. Left undetected, both PRs flow through the ordinary `R_new` → fix-checks-only → (after 3 attempts) `blocked:ci` path independently, and the real explanation — "these two unblock each other" — is never written down anywhere. The repro: two repo-wide breakages landed within hours of each other, each PR's fix living in the *other* PR's diff, stalling an 11-PR-deep merge train with no diagnosis in the record. See [RATIONALE → Mutually-blocking PR detection — why combining is out of scope](../do-work-RATIONALE.md#mutually-blocking-pr-detection--why-combining-is-out-of-scope-issue-1140) for the full reasoning.

**This is detection + surfacing only — it does not change dispatch routing.** `R_new` still flows into per-poll action 1 exactly as before; a detected pair still eventually reaches `blocked:ci` via the existing 3-attempt fix-checks circuit breaker if it stays red. What changes is that the state becomes **visible**: a loud, idempotent diagnosis comment lands on both PRs the first poll that detects the relationship, and the end-of-session summary carries a dedicated line. **Automated combining (cherry-picking the smaller diff onto the larger) is explicitly NOT implemented here** — it mutates a PR branch and, per the issue's own guardrail language, "changes what a reviewer is approving." That capability is left as a documented follow-up (a config-gated, default-off opt-in, should someone build it) — never a silent default.

**Candidate set.** Every poll, build the candidate PR-number list from `R_new ∪ B` (both already computed above) restricted to PRs whose `mergeStateStatus != "DIRTY"` (a DIRTY PR's checks are a frozen fossil of a stale base, not live evidence of a real cross-PR relationship — the same reasoning `R_new`'s own DIRTY exclusion uses). Skip the scan entirely when the candidate list has fewer than 2 members — there's no pair to find.
...
- **`mb_pairs` non-empty** → each stdout line is `<pr_a>,<pr_b>,<files_disjoint:0|1|unknown>`. For each pair:
  1. **Idempotency check — never re-post on every poll.** Before posting, check whether PR `<pr_a>` already carries a comment whose body starts with the sentinel `<!-- do-work-mutually-blocking pr-pair=<pr_a>,<pr_b> -->`:
     ```bash
     already_diagnosed=$(gh pr view <pr_a> --repo <owner/repo> --json comments \
       --jq "[.comments[].body] | any(startswith(\"<!-- do-work-mutually-blocking pr-pair=<pr_a>,<pr_b> -->\"))")
     ```
     If `true`, skip this pair entirely — it was already diagnosed (this session or a prior one) and re-posting on every 60s poll would spam the thread. If `false` (or the read fails, which resolves toward "not yet diagnosed" — a duplicate comment costs a thread ping, a missed one costs nothing since a later poll retries), continue to step 2.
  2. **Post the diagnosis comment on BOTH PRs.** Write the body with the `Write` tool to `$WORKTREE_PATH/.shipyard-scratch/mutually-blocking-<pr_a>-<pr_b>.md` (a heredoc `$(cat <<EOF ... EOF)` is refused per `shipyard:worker-preamble` § "Scratch directory") — seed `.shipyard-scratch/.gitignore` first if not already present — then post via `gh pr comment <M> --repo <owner/repo> --body-file <path>` on both `<pr_a>` and `<pr_b>`. Content:

     ```
     <!-- do-work-mutually-blocking pr-pair=<pr_a>,<pr_b> -->
     🔗 **Mutually-blocking pair detected** (issue #1140)

     This PR and #<other> each pass a required check the other is currently failing — neither can go green alone:

     - #<pr_a>: failing `<checks>`, passing `<checks>`
     - #<pr_b>: failing `<checks>`, passing `<checks>`

     <one of, depending on files_disjoint:>
       [1]       Their diffs touch disjoint files — a maintainer could resolve this by cherry-picking the smaller PR's commit(s) onto the larger and closing the smaller, or merging one manually once satisfied the other's fix is included.
       [0]       Their diffs touch overlapping files, so a mechanical combine isn't safe — this needs manual reconciliation.
       [unknown] Could not determine whether their diffs overlap — inspect both before combining.

     This is an automated diagnosis only; nothing has been merged, rebased, or closed. shipyard does not combine PRs automatically (see #1140).
     ```

     Best-effort `rm -rf "$WORKTREE_PATH/.shipyard-scratch"` after both comments post.
  3. **Log and count.** `[drain] mutually-blocking pair detected: PR #<pr_a> <-> PR #<pr_b> (files_disjoint=<0|1|unknown>) — diagnosis comment posted (#1140)`, and increment the session-local `mutually_blocking_pairs_detected` counter (deduped by unordered pair across the whole session — a pair diagnosed once, then re-observed on a later poll, does not double-count) for the [end-of-session summary](./cleanup-summary.md#end-of-session-summary).
- **`mb_pairs` empty and `mb_exit == 0`** → nothing to do; proceed to per-poll action 1 as normal.

**Why no dispatch-routing change.** A detected pair's PRs still flow through `R_new` → fix-checks-only → `blocked:ci` on the ordinary schedule. Skipping fix-checks-only dispatch for a diagnosed pair would save a dispatch or two, but it would also mean the pair never reaches `blocked:ci` on its own (nothing else stamps that label), keeping the drain alive until `max_drain_hours` instead of settling normally. That optimization — and the automated-combining capability the issue sketches — are both left as follow-up work rather than folded into this detection-and-surfacing slice.

**Per-poll actions.**

1. If `R_new > 0`: push `R_new` PRs onto `failed_prs` (deduped) and dispatch fix-checks-only workers against them — same `--concurrency` cap, same 3-attempt rule, same `blocked:ci` stamp on exhaustion that step A enforces. **`D_dirty_red` PRs are NOT dispatched here** ([#1060](https://github.com/mattsears18/shipyard/issues/1060), superseding [#577](https://github.com/mattsears18/shipyard/issues/577)) — a DIRTY PR (red or not) has no merge ref, so a fix-checks-only dispatch against it would immediately trip that mode's own DIRTY short-circuit and bail with `dirty #<M>` without attempting anything; `R_new`'s own `mergeStateStatus != "DIRTY"` exclusion already guarantees no `D_dirty_red` member reaches this action. Drain runs the same dispatcher logic step C uses, just with `failed_prs` as the only queue that's still drainable (no new issue work, no diverts; `divert_queue` is intentionally NOT re-evaluated during drain — a red main mid-drain becomes next session's problem because dispatching a fix-main-ci agent here would extend the session indefinitely). **Before each dispatch, run the [pre-dispatch head-branch reap](#pre-dispatch-head-branch-reap-self-pid-lock-release) against that PR's head branch** so the fresh fix-checks worker's `git switch <head>` doesn't collide with a self-PID-locked worktree from the originating worker (#370).
2. If `D_dirty > 0`: dispatch a fix-rebase worker for each, **subject to the `--concurrency` cap** (combined fix-checks + fix-rebase in-flight count must not exceed `--concurrency`) **AND the CI-minute config gates from issue [#323](https://github.com/mattsears18/shipyard/issues/323)**. `D_dirty` includes every `D_dirty_red` member — DIRTY-and-red PRs are dispatched here identically to any other DIRTY PR, since `fix-rebase.md` step 2 no longer bails on a hard check failure while DIRTY (see [RATIONALE](../do-work-RATIONALE.md#1060-supersedes-577--dirty-and-red-routes-to-fix-rebase-not-fix-checks)). Emit `[drain] PR #<M> DIRTY+failing → fix-rebase (not fix-checks); routing via D_dirty_red (#1060, supersedes #577)` for each `D_dirty_red` member to make the routing decision visible in the drain log.
   ...
   **In-process resolution attempt FIRST, before the pre-dispatch reap, the CI-minute gates, or any worker dispatch (issue [#1377](https://github.com/mattsears18/shipyard/issues/1377)).** Allocating the coordinated manifest version at dispatch time rather than land time means, on a `version_coordination`-enabled repo, most `D_dirty` PRs are DIRTY for exactly one deterministic, mechanical reason: a sibling PR merged first and advanced the manifest `.version` row + CHANGELOG top entry past what this PR pre-allocated — precisely the case [`fix-rebase.md` §4.6](../../agents/issue-worker/fix-rebase.md#46-version-coordinated-manifest--changelog-re-number--trivial-resolution-issue-466) already resolves deterministically. Paying for a full worker dispatch (a fresh isolated worktree, an agent turn, a model call) to run that two-line renumber is expensive — one measured session spent 8 of 25 dispatches (~32%, ~1.11M input tokens) on exactly this. For each `D_dirty` PR, before anything else in this action, try [`scripts/resolve-manifest-only-dirty.sh`](../../scripts/resolve-manifest-only-dirty.sh): it runs the same rebase-and-resolve logic §4.6 documents, but in-process from the orchestrator's own worktree against a short-lived, ephemeral `git worktree add --detach` — no fresh agent worktree, no worker dispatch. It also resolves the (cheaper, more common than it sounds) case of a DIRTY PR whose rebase turns out conflict-free once actually attempted, with no manifest/CHANGELOG config needed at all. Because it never claims the PR's branch NAME (a detached-HEAD checkout, never `git switch`), it does **not** need the pre-dispatch head-branch reap below first — a live lock on the branch from another worker can't block it:
   ...
   resolve_result=$("$CLAUDE_PLUGIN_ROOT/scripts/resolve-manifest-only-dirty.sh" resolve \
     --repo <owner/repo> --pr <M> --head-ref "$head_ref" --default-branch <default-branch> \
     --manifest "$vc_manifest" --version-jq "$vc_version_jq" --changelog "$vc_changelog")
   resolve_status=$?
   ```

   Reuse the `vc_manifest` / `vc_version_jq` / `vc_changelog` reads from [dispatch-rules.md's next-available-version computation](./dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) (when `vc_enabled != "true"`, pass empty strings for all three — the script still attempts a plain clean rebase with no config at all). Parse the result:

   - **`resolve_status == 0`** (stdout `resolved pr=<M> version=<v> head=<sha>`) — the DIRTY PR was rebased and force-pushed in-process. Treat this **exactly** like a `rebased #<M>` fix-rebase worker return per [steady-state.md's A.1 reconcile](./steady-state.md#a1-parse-the-return-string): log `[drain] PR #<M> resolved in-process via resolve-manifest-only-dirty.sh (v<v>) — no worker dispatched (#1377)`, increment `rebase_success_counts[<M>]`, and **skip** the CI-minute gates below, the pre-dispatch head-branch reap, and the fix-rebase dispatch for this PR this poll. Do NOT add `<M>` to `rebase_blocked_prs`.
   - **`resolve_status != 0`** (stdout `deferred reason=<code>: <detail>`) — could not resolve in-process (the PR isn't DIRTY anymore, a non-trivial conflict, a safety-net check tripped, or the push lease was rejected by a concurrent push). Nothing was pushed and nothing was left mid-rebase or mid-conflict — the script's own trap cleans up its ephemeral worktree on every exit path. Log `[drain] PR #<M> resolve-manifest-only-dirty.sh deferred (<reason>) — falling back to fix-rebase dispatch (#1377)` and fall through to the pre-dispatch head-branch reap, the CI-minute gates, and the fix-rebase dispatch below, unchanged from before this shortcut existed.

   **Before each fix-rebase dispatch that still proceeds** (i.e. every `D_dirty` PR the in-process attempt above deferred on), **run the [pre-dispatch head-branch reap](#pre-dispatch-head-branch-reap-self-pid-lock-release) against that PR's head branch** (#370) — this is the load-bearing call for fix-rebase, the mode `dont.md` confines to drain and the one the #370 repro caught bailing on every self-PID-locked PR.
...
4. Print one drain status line per poll:
   ...
   `dirty_red=<D_dirty_red>` surfaces PRs that are both DIRTY and failing so the operator can see at a glance which of the `dirty=` PRs being routed to fix-rebase are also carrying a stale, frozen check failure that the rebase itself will supersede once a merge ref exists again ([#1060](https://github.com/mattsears18/shipyard/issues/1060) — `D_dirty_red` is a subset of `D_dirty`, not a separate routing destination). Omit the `dirty_red=` token when `D_dirty_red == 0` to keep the line terse on sessions where no DIRTY-AND-red PRs appear.
...
### Post-main-CI-fix branch-refresh (drain phase) ([#993](https://github.com/mattsears18/shipyard/issues/993))

Per-poll action 1 above deliberately does **not** re-evaluate `divert_queue` during drain — a red `main` mid-drain is left for the next session rather than dispatching a fresh `fix-main-ci` worker that would extend this one indefinitely. But a `fix-main-ci` PR that was **already in flight** when drain began (dispatched during the steady-state loop, still running as the dispatch pool emptied) completes via the normal [step A reconcile](./steady-state.md#a-reconcile-the-return) regardless of `draining` state. Once that PR merges and `main`'s post-merge run goes green, every *other* `session_prs` entry that recorded a required-check `FAILURE` while `main` was still broken is now permanently stuck — GitHub does not re-run an already-completed check just because the base branch changed underneath it, so `mergeStateStatus` reads `MERGEABLE`, auto-merge is armed, and nothing ever re-triggers the stale check. Left unhandled, this strands the whole merge train for the rest of the drain (and beyond, into the next session) even though the blocking problem is already solved.

**Per-poll action (runs every poll, in addition to actions 1–4 above; read-only against `divert_queue` — never enqueues a new diversion).** Run the same **main-CI-status read** [setup.md 4.5a](./setup/04-backlog-divert.md#45-divert-checks-main-ci--pr-pileup) performs (the `gh run list` scan + branch-protection probe, aggregated to `main_ci.status`), but skip its enqueue / attempt-counter bullets entirely — drain only needs the health *signal*, not the divert decision. Detect a transition exactly as 04-backlog-divert.md does: compare against the `main_ci.status` this same read produced on the *previous* poll (absent on the first drain poll — treat as non-green). On a genuine transition into `green`, run [04-backlog-divert.md's post-main-CI-fix branch-refresh mechanism](./setup/04h-post-main-ci-branch-refresh.md#post-main-ci-fix-branch-refresh--un-stick-session-prs-carrying-a-stale-failing-required-check-993) verbatim against every OPEN, non-DIRTY PR in `session_prs` — same `fix_commit_date`/`fix_commit_sha` cutoff, same `stale_check_refresh_done` per-PR-per-SHA dedup map (carried across polls the same way `rebase_blocked_prs` and `rebase_success_counts` already are), same single-call-per-PR-per-fix guard, same DIRTY skip (that's `D_dirty`/`D_dirty_red`'s territory, not this gate's). This is the identical mechanism setup / step D use; drain's only difference is that it runs the read every poll instead of once at setup or once per step-D tick, and it never populates `divert_queue`.

A PR that gets its stale check re-triggered here re-enters the normal drain classification on the *next* poll — its rollup is now `PENDING`/`IN_PROGRESS` instead of a stale `FAILURE`, so it drops out of any red-PR bucket and is tracked by the ordinary `P_settled` / merge-watch path like any other in-flight PR. No special-case bookkeeping beyond the `stale_check_refresh_done` dedup map above is needed.
...
The default drain protocol assumes **cloud CI auto-runs on every PR push** — push → checks run → rollup goes green → `--auto` merges. On a **local-only-CI repo** that assumption is false: the merge-blocking commit status is posted by a **manually-run command** (e.g. a `local-ci` status, posted only after a session runs a local test/report command), not by cloud CI. Nothing posts the gate status on push, so every shipped PR sits OPEN with `--auto` armed but **never merges** — the dispatch loop "succeeds" (PRs shipped) while the backlog never actually drains. This section makes the merge-gate loop first-class instead of leaving it to be improvised per session. See [RATIONALE → Local-only-CI merge gate](../do-work-RATIONALE.md#local-only-ci-merge-gate-643) for the #643 repro that motivated it.
...
This pattern is documented here, not re-invented per session — see [RATIONALE → Local-only-CI merge gate](../do-work-RATIONALE.md#local-only-ci-merge-gate-643) for why (the #643 session had to improvise it live).
...
A repo that produces **release PRs** — a release-please `chore(main): release …` PR, an aggregated release-bump PR, or any PR whose sole job is to cut a version + CHANGELOG entry and (on merge) trigger a deploy — needs someone to arm auto-merge on them once their blockers land. Under the repo's **standing authority** (the [Permissions section](../../../../CLAUDE.md) grants landing on `main` via PR + auto-merge without asking on this personal-tooling repo, and the equivalent grant on any repo whose `auto_merge.policy` is not `never`), drain arms auto-merge on the release PR and drives it to merged → deployed, then picks up the next release PR for the following batch. See [RATIONALE → Release-PR auto-arming and deploy watch](../do-work-RATIONALE.md#release-pr-auto-arming-and-deploy-watch-659--663) for why this is drain's job rather than a human's.
...
Closes the orchestrator-turn half of [#720](https://github.com/mattsears18/shipyard/issues/720). On an **ungated** repo shape (`gh pr merge --auto` doesn't queue — it direct-merges immediately; see [`detect-ungated-admin-direct-merge.sh`](../../scripts/detect-ungated-admin-direct-merge.sh)), the four orchestrator-turn call sites that would otherwise arm `--auto` — [inline-trivial §E](./inline-trivial.md#e-arm-auto-merge), the [A.0.5 crash-recovery re-arm](./steady-state.md#a05-post-return-worktree-reap-for-crashed--narrative-non-terminal-returns-fires-before-a1s-return-string-parsing), the [setup-3c orphan-recovery re-arm](./setup/00-config-worktree.md#07-setup-parallelization-contract-fire-once-batch), and the [release-train sweep](#release-pr-auto-arming-and-deploy-watch-own-the-tail-phase-c--663) above — deliberately **leave the PR OPEN and unarmed** rather than merging it ungated. They cannot re-create the gate by blocking on `gh pr checks --watch` the way a worker does ([fix-main-ci step 7.a](../../agents/issue-worker/fix-main-ci.md), [issue-work §6.a](../../agents/issue-worker/issue-work.md#6-enable-auto-merge-gated-on-originating_author_trust)), because they run on the **orchestrator's own turn** — a multi-minute block there stalls the dispatch loop, every in-flight reconcile, and every other PR's progress.
...
This gap is documented but deliberately not closed — two larger options would close it but expand drain's behavioral surface materially, and both are tracked as follow-up work to [#359](https://github.com/mattsears18/shipyard/issues/359) rather than implemented here. See [RATIONALE → Systemic CI failure during drain — options considered](../do-work-RATIONALE.md#systemic-ci-failure-during-drain--options-considered-359) for what they are and why neither was picked.
