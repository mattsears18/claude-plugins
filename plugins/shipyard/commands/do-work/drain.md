...
**Per-poll action (runs every poll, in addition to actions 1–4 above; read-only against `divert_queue` — never enqueues a new diversion).** Run the same **main-CI-status read** [setup.md 4.5a](./setup/04-backlog-divert.md#45-divert-checks-main-ci--pr-pileup) performs (the `gh run list` scan + branch-protection probe, aggregated to `main_ci.status`), but skip its enqueue / attempt-counter bullets entirely — drain only needs the health *signal*, not the divert decision. Detect a transition exactly as 04-backlog-divert.md does: compare against the `main_ci.status` this same read produced on the *previous* poll (absent on the first drain poll — treat as non-green). On a genuine transition into `green`, run [04-backlog-divert.md's post-main-CI-fix branch-refresh mechanism](./setup/04-backlog-divert.md#post-main-ci-fix-branch-refresh--un-stick-session-prs-carrying-a-stale-failing-required-check-993) verbatim against every OPEN, non-DIRTY PR in `session_prs` — same `fix_commit_date`/`fix_commit_sha` cutoff, same `stale_check_refresh_done` per-PR-per-SHA dedup map (carried across polls the same way `rebase_blocked_prs` and `rebase_success_counts` already are), same single-call-per-PR-per-fix guard, same DIRTY skip (that's `D_dirty`/`D_dirty_red`'s territory, not this gate's). This is the identical mechanism setup / step D use; drain's only difference is that it runs the read every poll instead of once at setup or once per step-D tick, and it never populates `divert_queue`.
