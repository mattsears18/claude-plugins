...
**Latest-per-name semantics for `R_new`, `D_dirty_red`, and `D_dirty`.** `statusCheckRollup` returns every check run for the PR's head SHA — including superseded runs (a check that ran, failed, was re-triggered, and passed appears twice). De-duplicate by check name and take the most recent entry per check (by `completedAt`, fallback `startedAt`) before computing any of the three sets below. See [RATIONALE → Latest-per-name rollup de-duplication](../do-work-RATIONALE.md#latest-per-name-rollup-de-duplication-333) for the naïve-walk failure modes this prevents and the session repro that motivated it.
