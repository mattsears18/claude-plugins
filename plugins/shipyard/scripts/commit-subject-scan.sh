...
# starts with the word (`Merge the two code paths`) is NOT exempted — the
# trailing keyword is what makes these git-generated.
MERGE_RE='^Merge (branch|pull request|remote-tracking branch|tag|commit) '
REVERT_RE='^Revert "'
