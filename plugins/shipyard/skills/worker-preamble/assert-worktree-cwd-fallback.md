On-demand fragment of the `shipyard:worker-preamble` skill (see [`SKILL.md`](./SKILL.md) § "Step-0 cwd fail-fast" and § "Mid-session cwd anchoring"). It carries the git-dir-vs-git-common-dir comparison both of those sections use, in both call shapes (step-0's bare form and mid-session's `-C`-anchored form), so the always-loaded core doesn't duplicate them. Claude Code enforces the same rule itself — a command whose working directory resolves to the main checkout is blocked before it runs — so this check is a self-confirmation, not the only thing standing between you and a wrong-repo write.
