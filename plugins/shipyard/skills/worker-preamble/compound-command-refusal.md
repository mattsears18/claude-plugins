# Worker-preamble fragment — When the #829 re-block loop is itself refused

On-demand fragment of the `shipyard:worker-preamble` skill (see [`SKILL.md`](./SKILL.md) § "A foreground call the harness auto-backgrounds past 600s"). Load this the moment the sanctioned re-block loop — `until [ -s "<output-file>" ]; do sleep 15; done; cat "<output-file>"`, or an equivalent read-only polling loop against a live command like `gh run view` — is itself refused by Auto Mode's compound-command classifier. Most dispatches never hit this: it fires only inside a worktree-isolated session, only for a compound shell loop, and only when Auto Mode's classifier decides the loop is "too complex to verify" containment.
