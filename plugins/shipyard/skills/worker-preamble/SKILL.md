...
**Forbidden by default.** You have your own isolated worktree and your own branch — there is never a legitimate reason to stash. Don't reach for `git stash`, `git stash pop`, or a bare `git stash apply` as a way to shelve local changes mid-task.

**The mechanism, so this is understood rather than merely obeyed (and so a future editor doesn't relax it).** Worktree isolation covers *checkouts* — every worker's `git worktree add`'d directory has its own working tree and its own branch. It does **not** cover `git stash`: every stash entry lives under `refs/stash` in the shared `.git` — the **common** dir every linked worktree shares (the same `git-common-dir` the [step-0 cwd fail-fast](#step-0-cwd-fail-fast--assert-youre-actually-in-your-worktree-486) reads to detect a primary-checkout mispin). So the stash stack is not per-worktree, not per-branch, not per-dispatch — it is one shared LIFO stack visible to and mutable by **every concurrent worker in the repository**. A bare `git stash pop` always takes `stash@{0}`, regardless of who pushed it — on any session running `--concurrency > 1`, popping is a coin flip over whichever worker pushed most recently.
...
**If stashing is genuinely unavoidable** (rare enough that you should be able to name why your own worktree + branch aren't sufficient before reaching for this), use the isolating form and never the bare pop:
...
**The check — one script invocation, not a compound `if` + command-substitution snippet ([#826](https://github.com/mattsears18/shipyard/issues/826)).** The worktree-isolation guard refuses a single call built from command substitutions, `cd` subshells, and an inline `if` — exactly the shape a one-shot version of this check takes. [#802](https://github.com/mattsears18/shipyard/issues/802) first fixed that by splitting the check into three separate plain `git rev-parse` calls with the worker doing the comparison itself; [#826](https://github.com/mattsears18/shipyard/issues/826) finishes the job by extracting the comparison into [`scripts/assert-worktree-cwd.sh`](../../scripts/assert-worktree-cwd.sh) — the **single executable source of truth** for this predicate, shared with [Mid-session cwd anchoring](#mid-session-cwd-anchoring--the-step-0-check-is-not-a-per-call-guarantee-748) below so the rule can no longer drift between the two call sites the way [#716](https://github.com/mattsears18/shipyard/issues/716)'s ungated-merge rule drifted before `detect-ungated-admin-direct-merge.sh` fixed it. The predicate itself is unchanged: a linked worktree has a *separate* per-worktree git dir (`<common>/worktrees/<name>`) distinct from the common dir, so `git rev-parse --git-dir` and `git rev-parse --git-common-dir` differ there. The PRIMARY checkout has git-dir == git-common-dir (both resolve to the same `.git`) — that equality is the load-bearing signal: true **only** for a primary checkout and false for every linked worktree, regardless of where under the tree the cwd sits.

**Check your dispatch prompt for an orchestrator-supplied literal plugin root FIRST ([#965](https://github.com/mattsears18/shipyard/issues/965)).** Look near the top of the prompt for language like *"Resolved plugin root (orchestrator-supplied, literal path): `<path>`"* (`Agent`-tool shape) or a `pluginRoot` field on your work unit (`Workflow`-substrate). If present, **use that literal path directly** in place of `${CLAUDE_PLUGIN_ROOT}` everywhere in this skill and in `agents/issue-worker/<mode>.md` (mid-session anchoring, the CHANGELOG monotonicity scan, the verify_gate model resolution, §6.a's ungated-admin-direct-merge detector) and skip the resolution block below entirely — resolve once, reuse everywhere. Only self-resolve, below, when your prompt supplied no literal path (a hand dispatch, or a caller predating #965) — `${CLAUDE_PLUGIN_ROOT}` is not reliably set in Bash-tool subprocesses (issue [#354](https://github.com/mattsears18/shipyard/issues/354)). This is the canonical idempotent re-export preamble used everywhere else in this plugin — **it is a compound command the harness's Auto Mode classifier can refuse as "too complex to verify that it stays inside the worktree" ([#965](https://github.com/mattsears18/shipyard/issues/965)); if refused, skip straight to the decomposed fallback further down rather than retrying it**:
...
**If the script can't be located** (an installed plugin predating [#826](https://github.com/mattsears18/shipyard/issues/826), the `CLAUDE_PLUGIN_ROOT` resolution above comes back empty, **or the resolution command itself was refused by the harness's Auto Mode classifier** — issue [#965](https://github.com/mattsears18/shipyard/issues/965), the compound export is a command-substitution-plus-inline-`if` shape the classifier can reject as "too complex to verify that it stays inside the worktree"), fall back to the pre-#826 three-plain-calls form below — it needs no `CLAUDE_PLUGIN_ROOT` at all, so it is unaffected either way. Issue each as its **own** `Bash` tool call; do not paste them together and chain them with `&&`, a subshell, or an `if`:
...
**If the script can't be located**, fall back to the pre-#826 form — issue each as its own plain `Bash` call, using the literal `WORKTREE_PATH` in place of `$WORKTREE_PATH`:
...
Compare the two outputs by inspection, exactly as in step-0's fallback. If they're the same path, the anchor has drifted — same `blocked:` wording as above.
...
This is compliant for three reasons: it is itself a **foreground** call, so it blocks your turn exactly like the original command would have; it self-heartbeats via the `until`/`sleep` loop (so the stream watchdog in [`ci-pitfalls.md`](./ci-pitfalls.md) never fires); and it cannot outlive your return, so it satisfies #529 and #707 alike. It is also the "wait for a condition, not a fixed sleep" shape the harness's own guard steers toward when it blocks a naive `sleep N; tail …` retry — that guard is pointing you at this pattern, not at `Monitor` or `run_in_background: true`, even though its error text can read that way.
