...
**Re-validated against native background auto-resume ([#785](https://github.com/mattsears18/shipyard/issues/785)) — complementary, not a replacement.** Claude Code's background subagents now survive their own process crashing or the harness restarting: a supervisor process persists session state to disk and auto-resumes a background subagent from its own transcript/checkpoint after such an event. That durability layer sits **underneath** this escape hatch, not on top of it, because the two guard against different failure modes:

- **Harness auto-resume** protects against the *Claude Code process itself* dying or restarting mid-dispatch (a crash, an update, the machine sleeping) — the subagent's conversation and in-flight background work resume automatically, with no worker-side action needed.
- **This escape hatch** protects against the *worktree directory itself* being removed out from under a still-running worker — the orchestrator's end-of-session cleanup (or a user's manual `git worktree prune`) deleting `.claude/worktrees/agent-*` while a dispatch is still active. Auto-resume restores the *session*, but a resumed session whose worktree directory no longer exists on disk is in exactly the state this fragment detects: `git rev-parse --show-toplevel` fails or points somewhere else, and the `reaped:` bail fires correctly.

So auto-resume is a net reliability improvement (fewer dispatches lost outright to a harness-side crash) but does not subsume this guard — a worker still needs to re-derive `WORKTREE_PATH` before every write, because the specific failure mode this fragment exists for (the worktree directory is gone, regardless of why the session resumed) is orthogonal to whether the *session* itself survived a restart. No code change follows from this re-validation; the existing re-derive-before-write pattern above remains the correct, sufficient guard.
