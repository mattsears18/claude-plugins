...
Committing the **shape first**, behind a flag, let migration proceed against a stable target rather than designing the substrate and the migration in the same PR — and let the cutover ship reversibly, with a one-line config revert, before the legacy path was finally removed. (Phase-by-phase detail: RATIONALE link above.)

## Contents

| File | What it is |
|---|---|
| [`do-work-dispatch.workflow.js`](./do-work-dispatch.workflow.js) | **GENERATED — do not hand-edit.** The dispatch-loop script the Dynamic Workflows runtime actually executes: `select issues → dispatch (build the mode-specific prompt) → collect`, plus every one of the seven `mode:` values' full prompt builder (`buildIssueWorkPrompt`, `buildFixChecksOnlyPrompt`, `buildFixRebasePrompt`, `buildFixMainCiPrompt`, `buildFixFailingPrsBatchPrompt`, `buildInvestigatePrompt`, `buildSpikePrompt`), inlined by `../scripts/generate-dispatch-workflow.mjs` from the two source rows below. Checked into git as the generated artifact — the runtime has no filesystem access of its own, so it cannot `import` the sources at execution time (issue [#958](https://github.com/mattsears18/shipyard/issues/958)). `../scripts/tests/dispatch-workflow-generated-958.test.sh` fails CI if this file has drifted from what regenerating the sources produces. |
| [`do-work-dispatch.core.js`](./do-work-dispatch.core.js) | **Source of truth** for the orchestration logic half: `meta`, `args` handling (including the fail-loudly contract, issue #817), the select/dispatch/collect stages, `unitLabel`, the `buildWorkerPrompt` mode-routing switch, and `workerReturnSchema`. Edit here, then run `node ../scripts/generate-dispatch-workflow.mjs` to regenerate `do-work-dispatch.workflow.js`. All of the historical/runtime-API documentation that used to sit at the top of the generated file now lives in this file's header comment. |
| [`prompt-templates/`](./prompt-templates/) | **Source of truth** for the prompt-builder half: `shared.mjs` (the `worktreeAnchorLines`/`worktreeAnchorCheckLines` helpers every mode delegates to) plus one file per mode (`issue-work.mjs`, `fix-checks-only.mjs`, `fix-rebase.mjs`, `fix-main-ci.mjs`, `fix-failing-prs-batch.mjs`, `investigate.mjs`, `spike.mjs`) — each mirroring its dispatch-rules.md prompt template, including that mode's augmentations (verify-gate / user-feedback / phase-1-slice / version-coordination for issue-work; the version-coordination §4.6 carve-out for fix-rebase; the triage.auto_close policy for investigate; the decompose.max_subissues fan-out cap for spike). Every file here is a normal, valid, independently `node --check`-able ES module — unlike the generated file or `do-work-dispatch.core.js`, which only make sense once concatenated. Edit here, then regenerate (see above). |
| [`../scripts/generate-dispatch-workflow.mjs`](../scripts/generate-dispatch-workflow.mjs) | The generator: concatenates `do-work-dispatch.core.js` + `prompt-templates/*.mjs` (stripping `import`/`export` syntax) into `do-work-dispatch.workflow.js`. Run with no args to regenerate in place, or `--check` to verify (no write) — the mode CI uses. |
| [`../schemas/worker-return.schema.json`](../schemas/worker-return.schema.json) | The structured-return schema each `agent()` call validates against — the schema-validated return contract for every workflow-dispatched worker (see [`worker-preamble` § "Return-contract discipline"](../skills/worker-preamble/SKILL.md)). Mirrored (by hand, not imported — the workflow runtime executes this script with no filesystem access of its own) as `workerReturnSchema` inside `do-work-dispatch.core.js`; keep the two in sync on any return-contract change. A drift on any validation-relevant facet (`enum`/`required`/`additionalProperties`/`allOf`/`minimum` — `description` text excluded) is caught mechanically in CI by [`../scripts/check-worker-return-schema-parity.mjs`](../scripts/check-worker-return-schema-parity.mjs) (run via `scripts/tests/worker-return-schema-parity-856.test.sh`), so "keep in sync by hand" is no longer the only line of defense — see issue [#856](https://github.com/mattsears18/shipyard/issues/856). |

**Editing this directory:** change `do-work-dispatch.core.js` and/or a `prompt-templates/*.mjs` file, then run `node ../scripts/generate-dispatch-workflow.mjs` and commit the regenerated `do-work-dispatch.workflow.js` alongside your source edit. Never edit `do-work-dispatch.workflow.js` directly — the drift-check test will catch it, but the point of the split is to make the sources the pleasant place to work, not to catch mistakes after the fact.
