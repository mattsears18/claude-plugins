...
for safety in guard-primary-checkout refuse-escape-symlink-commit refuse-broad-process-kill refuse-credential-mint refuse-unsafe-git-stash refuse-hook-bypass-flag refuse-irreversible-external-mutation; do
...
# The Bash matcher carries six refusal hooks — assert they all compose
# (hooks append, they don't replace).
bash_commands=$(jq -r '
  [ .hooks.PreToolUse[]? | select(.matcher == "Bash") | .hooks[].command ] | .[]
' "$hooks_json")
bash_ok=1
for bh in refuse-escape-symlink-commit refuse-broad-process-kill refuse-credential-mint refuse-unsafe-git-stash refuse-hook-bypass-flag refuse-irreversible-external-mutation; do
  if ! printf '%s\n' "$bash_commands" | grep -q "/${bh}\.sh\""; then
    bash_ok=0
  fi
done
if [[ $bash_ok -eq 1 ]]; then
  ok "Bash matcher composes all six refusal hooks (appended, not replaced)"
else
  no "Bash matcher lost one of its six refusal hooks"
fi
# -----------------------------------------------------------------------------
echo "== Summary"
# -----------------------------------------------------------------------------
