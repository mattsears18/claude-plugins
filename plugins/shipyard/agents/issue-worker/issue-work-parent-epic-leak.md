...
Shapes (1)–(3) run this procedure to *prevent* a closing link from ever registering. Shape (4) is the inverse starting condition: `#<N>` already registered as a real closing link via §5.8's normal, correct flow, and a later finding in the same dispatch — most often a post-open validation/landing-gate check disproving the issue's premise — determined the link must come out. **Run the exact same procedure above, with `#<E> = #<N>`.** The script doesn't distinguish where the protected issue number came from; nothing about it needs to change for shape (4).
