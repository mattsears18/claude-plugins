...
1. **Pre-flight: re-confirm main is still red.** State drifts between dispatch and you starting. The green-main assertion is a **script, not a rule for you to re-derive** — `noop: main already green` is a claim that CI passed, and it must never be reachable from a check that observed nothing (issue [#717](https://github.com/mattsears18/shipyard/issues/717); see `shipyard:worker-preamble` § "An absence-assertion that observed nothing is not a pass" — fragment [`ci-pitfalls.md`](../../skills/worker-preamble/ci-pitfalls.md)):
   ...
   - **`1` (red)** → main is still red as dispatched. Proceed to step 2. Note the script may reach `red` via its step-level walk-back ([#1495](https://github.com/mattsears18/shipyard/issues/1495)) — i.e. main's *newest* run reported success but skipped every real step, and the last run that actually executed failed. Its stderr names that run's id; **that** run, not the newest one, is the one whose logs are worth reading in step 2, and it may be older than the `<earliest_red_run_id>` you were dispatched with.
   ...
   - **`4` (unproven)** → runs matched and reported success, but every substantive step was `skipped` by a `paths`/`paths-ignore` path filter, and no run within the lookback window actually executed ([#1495](https://github.com/mattsears18/shipyard/issues/1495)). The success carries **no evidence** about the tree. Treat exactly like `unknown` — *not verified*, never the noop. Proceed to step 2 on the dispatch's premise; if step 2's logs are also unreadable, return `blocked main-ci-fix: could not read CI state for <default-branch> (newest runs are path-filter skips carrying no signal)`.

   Do NOT hand-roll this with a `gh run list ... --json conclusion --jq '[.[] | select(.conclusion != "success")] | length'`-style absence-assertion: on an empty result set that predicate returns 0 and reads as green, so a lookup that matched no runs would produce a false `noop: main already green` and leave the red branch unrepaired. The same applies one layer down — do NOT substitute a `jobs[].conclusion` read for the script's `jobs[].steps[]` walk: a required job that skipped every real step still reports `conclusion: success`, so `noop: main already green` would again be reachable over a live red ([#1495](https://github.com/mattsears18/shipyard/issues/1495)).
...
8. **Return one line** — synchronously, after the work reaches its real end state. Per `shipyard:worker-preamble` § "Return-contract discipline" ([#529](https://github.com/mattsears18/shipyard/issues/529)), do NOT arm a `run_in_background` process / `Monitor` / background-waiter and return a non-terminal narrative (e.g. *"I'll wait for the notification"*) before it resolves — that reports the dispatch complete while the work is stranded. Block your own turn on the foreground command if you must wait, then return exactly one of:
   ...
   - `shipped main-ci-fix via PR #<M> (auto-merge: unarmed — gate-narrowing: <signal>, needs-human-review label applied, checks: <green|pending|failing>)` — the step-0.34 gate-narrowing check tripped ([#1139](https://github.com/mattsears18/shipyard/issues/1139)): your fix narrowed a required CI gate (e.g. a new allowlist, a raised severity threshold) rather than fixing the underlying cause. The fix still shipped as a PR — it just doesn't auto-merge; a human accepts the risk.
