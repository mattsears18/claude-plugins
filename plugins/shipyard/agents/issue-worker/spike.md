...
- **Don't define or hardcode spike-detection heuristics in the orchestrator's dispatch path.** That routing logic (label / title-framing detection, dispatch-site wiring in `commands/do-work/dispatch-rules.md` / `steady-state.md`) is [#774](https://github.com/mattsears18/shipyard/issues/774)'s scope, not this file's. This mode assumes it has already been dispatched against a specific issue. The detector itself is [`scripts/spike-shape-detect.sh`](../../scripts/spike-shape-detect.sh), the single source of truth as of [#1475](https://github.com/mattsears18/shipyard/issues/1475) — if you need to know what "spike-shaped" means, read that script, not a prose copy.
