...
Detecting that a given issue is spike-shaped and choosing this shim over `shipyard:issue-worker` at dispatch time is wired into the `ready_issues` dispatch site in [`commands/do-work/dispatch-rules.md`](../commands/do-work/dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) (step 3's spike-shape check, ahead of the normal `mode: issue-work` prompt composition) — see [issue #774](https://github.com/mattsears18/shipyard/issues/774). `commands/do-work/steady-state.md`'s step A.1 reconciles this shim's return-string vocabulary (`spiked+shipped` / `spiked+needs-human-review`) alongside the other modes'.

**The detector is [`scripts/spike-shape-detect.sh`](../scripts/spike-shape-detect.sh), and it is the single source of truth** ([#1475](https://github.com/mattsears18/shipyard/issues/1475)). Two signals, OR'd: a `spike` **label**, or an anchored, case-insensitive **title prefix** — either a Conventional-Commits *type* (`spike`, `investigate`, `research`, `feasibility`) with an **optional `(scope)` group**, so `spike(ci):` matches as well as `spike:`, or one of the two bare prose framings (`spike on`, `design spike:`), which get no scope group because they aren't CC types. That paragraph is a **non-normative** description of the script, not a second copy to re-derive: the previous hand-maintained wording here enumerated only the bare forms, which is exactly how [#1474](https://github.com/mattsears18/shipyard/issues/1474) — titled `spike(ci): …`, and spike-shaped by its own title word — was missed. #1475 also records the decision for `investigate(…)`, which collides with the separate investigate **mode**: the investigate mode wins whenever its own detection fires, because it runs an entire phase earlier (setup step 4) and removes the issue from the pool before this check runs; see [dispatch-rules.md's spike-shape detection block](../commands/do-work/dispatch-rules.md#dispatch-rules-used-by-step-7-and-step-c) for the full reasoning.
...
See `shipyard:mode-shim-preamble` § "Mode → shim → model mapping" for the general rationale (one per-mode file per shim keeps the router thin and each mode's context load scoped to what that mode needs) and the full table. This shim's own row carries no model pin — see "Why no model pin" above for the reasoning specific to this mode. Same pattern as the five cost-optimized shims — a dedicated `agents/issue-worker/spike.md` per-mode file, referenced by this shim, rather than a branch inside `issue-work`'s.
